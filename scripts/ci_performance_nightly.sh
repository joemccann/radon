...
# phase then the remediate phase, then the deliver phase (push, PR, CI green,
...
# `remediate` / `deliver`). See .claude/skills/ci-performance/.
#
...
MODE="${1:?usage: ci_performance_nightly.sh audit|remediate|deliver|cycle}"
...
LOOP_MARKER=".radon-ci-performance-runner"
if [[ ! -f "$LOOP_MARKER" ]]; then
  if [[ "$(pwd -P)" == "$(cd "$HOME/radon-weekend/radon-ci-performance" 2>/dev/null && pwd -P)" ]]; then
    touch "$LOOP_MARKER"
  else
    echo "REFUSING: $REPO is not the dedicated ci-performance runner clone ($LOOP_MARKER absent)" >&2
    report "REFUSED" "$REPO lacks the $LOOP_MARKER marker; the ci-performance loop runs only in ~/radon-weekend/radon-ci-performance" || true
    exit 2
  fi
fi
...
run_phase() {
  begin_phase "$1"
  trap on_crash ERR
  echo "[ci-performance] $PHASE start $STAMP repo=$REPO cap=${CAP_SECS}s${BILLING_IGNORED:+ ignored=${BILLING_IGNORED// /,}}" | tee -a "$RUN_LOG"
  arm_deliver_record
  ...
  while :; do
    ...
    CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0 "$TIMEOUT_BIN" -k "$KILL_AFTER_SECS" "$remain" claude -p "/ci-performance $PHASE" \
    ...
    if (( RC != 0 )) && is_quota_exhausted; then
      if (( MODEL_INDEX + 1 < ${#MODEL_RUNGS[@]} )); then
        ...
        echo "[ci-performance] ${MODEL_RUNGS[$((MODEL_INDEX - 1))]} is exhausted; dropping to ${MODEL_RUNGS[$MODEL_INDEX]}" | tee -a "$RUN_LOG"
        continue
      fi
...
if [[ "$MODE" == "cycle" ]]; then
  ...
  RC_DELIVER=$RC
  set -e
  echo "[ci-performance] cycle done audit_rc=$RC_AUDIT remediate_rc=$RC_REMEDIATE deliver_rc=$RC_DELIVER"
  prune_weekend_root
