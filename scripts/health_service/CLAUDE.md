...
- `GET /caddy-tls-ask?domain=` — Caddy on-demand TLS permission. `200` only for `mcp.radon.run`, `404` otherwise. Bind is loopback; not on the public edge. Lives here so `systemctl reload caddy` does not wait on an ask site in the same process.
- `GET /status` — **ALWAYS `200`**, and **trust-split**. Degraded sources are body fields, never error codes (`feedback_http_status_for_real_errors.md`).
  ...
  **The app-role suppression is bounded, not permanent (REL-243, NF-10).** `effective_not_applicable()` keeps the gateway exclusion while the nested `radon-api:broker` probe is observed up, or while the local unit is a clean-inactive `Result=success` (true app host: unit disabled by the two-host split). A local crash (not `Result=success`) degrades to counted after the existing 900s `DEPENDENCY_DWELL_LIMIT_SECS`, so a misapplied `RADON_HOST_ROLE=app` on a host that IS running a local gateway cannot stay edge-green forever. On expiry `/status` sets `role_suppression_expired` and empties `not_applicable`. Nested broker-down on a true app host stays `degraded` via `radon-api:broker` and must not page `aggregate_down` (2026-09-15 page `3a6de316`).
