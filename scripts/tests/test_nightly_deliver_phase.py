...
# --------------------------------------------------------------------------
# (c) the skills carry the mandate
# --------------------------------------------------------------------------
class TestTheSkillsCarryTheMandate:
    @pytest.mark.parametrize("loop", LOOP_IDS)
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ready to merge",
            "nightly_deliver.py",
            "RADON_WEEKEND_DELIVER_CAP_SECS",
            "NIGHTLY DELIVER READY:",
            "NIGHTLY DELIVER INCOMPLETE:",
            "one dated branch",
            "before any long suite",
            "Never weaken a test or a gate to get green",
        ],
    )
    def test_the_mandate_sentence_is_present(self, loop, sentence):
        text = " ".join(_skill(loop).read_text(encoding="utf-8").split())
        assert sentence in text, f"{loop}: SKILL.md no longer states {sentence!r}"

    @pytest.mark.parametrize("loop", LOOP_IDS)
    @pytest.mark.parametrize(
        "metric",
        [
            "findings implemented per cycle",
            "PRs opened",
            "time to CI green",
            "PRs awaiting merge",
        ],
    )
    def test_the_metrics_are_stated(self, loop, metric):
        text = " ".join(_skill(loop).read_text(encoding="utf-8").split())
        assert metric in text, f"{loop}: SKILL.md does not measure {metric!r}"

    @pytest.mark.parametrize("loop", LOOP_IDS)
    def test_a_zero_fix_night_is_no_longer_the_default(self, loop):
        text = " ".join(_skill(loop).read_text(encoding="utf-8").split())
        assert "at most one" not in text, f"{loop}: still caps remediation at one fix"
        assert "one bounded, evidence-backed optimization per night" not in text
        assert "un-started work logged as `DEFERRED`" not in text
        assert "verified findings with no implementation" in text, (
            f"{loop}: SKILL.md must say a night with verified findings and no fix is a failed phase"
        )

    @pytest.mark.parametrize("loop", LOOP_IDS)
    def test_the_dated_branch_is_named(self, loop):
        text = " ".join(_skill(loop).read_text(encoding="utf-8").split())
        assert f"{loop}/<YYYY-MM-DD>" in text

    @pytest.mark.parametrize("loop", LOOP_IDS)
    def test_the_description_names_the_three_phase_cycle(self, loop):
        head = _skill(loop).read_text(encoding="utf-8").split("---")[1]
        assert "deliver" in head, f"{loop}: the frontmatter description does not name the deliver phase"
        assert "audit then remediate" not in head

    def test_security_keeps_p0_p1_private_in_the_deliver_phase(self):
        text = " ".join(_skill("security").read_text(encoding="utf-8").split())
        assert "P0/P1" in text and "released" in text
        assert "explicitly released" in text
        assert "no vulnerability detail" in text.lower() or "never a route, file, attack" in text.lower()

    def test_operator_only_findings_land_in_the_pr_next_section(self):
        for loop in LOOP_IDS:
            text = " ".join(_skill(loop).read_text(encoding="utf-8").split())
            assert "Next section" in text or "Next" in text
            assert "never silently dropped" in text or "Nothing is dropped silently" in text, loop

    def test_the_runbook_states_the_three_phase_cycle(self):
        ops = (REPO / "docs" / "operations.md").read_text(encoding="utf-8")
        assert "audit, remediate, deliver" in ops or "audit -> remediate -> deliver" in ops
        assert "ready to merge" in ops
        assert "RADON_WEEKEND_DELIVER_CAP_SECS" in ops
