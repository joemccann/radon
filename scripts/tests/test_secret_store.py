...
class TestRoundtrip:
    def test_set_then_get_returns_plaintext(self, store):
        store.set_secret("UW_TOKEN", "uw-abc123secret", actor="operator")
        assert store.get_secret("UW_TOKEN") == "uw-abc123secret"

    def test_get_missing_returns_none(self, store):
        assert store.get_secret("ANTHROPIC_API_KEY") is None

    def test_overwrite_replaces_value_and_bumps_version(self, store):
        store.set_secret("EXA_API_KEY", "first-value-1234", actor="operator")
        store.set_secret("EXA_API_KEY", "second-value-5678", actor="operator")
        assert store.get_secret("EXA_API_KEY") == "second-value-5678"
...
class TestListNeverLeaksPlaintext:
    def test_list_masks_value_to_last_four(self, store):
        ...
        assert "uw-abc123secret" not in str(entry)
    ...
    def test_list_carries_metadata(self, store):
        store.set_secret("UW_TOKEN", "uw-abc123secret", actor="op-1")
        (entry,) = store.list_secrets()
...
class TestEncryptionAtRest:
    def test_plaintext_absent_from_db_file(self, store, tmp_path):
        store.set_secret("UW_TOKEN", "uw-plainly-visible-token", actor="operator")
        raw = (tmp_path / "secrets.db").read_bytes()
        assert b"uw-plainly-visible-token" not in raw

    def test_ciphertext_bound_to_name(self, store, tmp_path):
        """AAD = name: swapping ciphertext rows must fail decryption."""
        store.set_secret("UW_TOKEN", "value-for-uw-1234", actor="operator")
        store.set_secret("EXA_API_KEY", "value-for-exa-5678", actor="operator")
    ...
    def test_second_store_instance_reuses_key(self, tmp_path):
        ...
        first.set_secret("UW_TOKEN", "uw-abc123secret", actor="operator")
        second = SecretStore(
            db_path=tmp_path / "secrets.db", key_path=tmp_path / "k.key"
        )
        assert second.get_secret("UW_TOKEN") == "uw-abc123secret"
...
class TestValidation:
    ...
    def test_invalid_names_rejected(self, store, bad_name):
        with pytest.raises(SecretValidationError):
            store.set_secret(bad_name, "some-value-1234", actor="operator")
