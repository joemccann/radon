...
# ── (f) read_service_health_http: bounded single-row SELECT ────────────────

class TestReadServiceHealthHttp:
    """T-100: the durable Flex embargo is read back over the same bounded
    transport that wrote it. Row → dict keyed by column; no row → None;
    transport failure → HranaHttpError (the caller owns fail-open)."""

    @staticmethod
    def _row_body(*cells: object) -> bytes:
                ...
                "results": [
                    {
                        ...
                        "response": {
                            "type": "execute",
                            "result": {"rows": [[_cell(c) for c in cells]]},
                        },
                    }
                ]
            }
    ...
    def test_row_comes_back_as_a_column_keyed_dict(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        ...
        def fake_urlopen(req, timeout=None):
            ...
            return _FakeHttpResponse(
                self._row_body(
                    "flex-web-service", "error", None, None,
                    '{"next_attempt_at": "2026-08-28T13:58:26Z"}',
                    "2026-08-21T13:58:26Z",
                )
            )

        monkeypatch.setattr(urllib.request, "urlopen", fake_urlopen)

        row = read_service_health_http("flex-web-service")
        ...
        assert row == {
            ...
            "last_error": '{"next_attempt_at": "2026-08-28T13:58:26Z"}',
            "updated_at": "2026-08-21T13:58:26Z",
        }
        sent = json.dumps(captured["body"])
        assert "FROM service_health" in sent
        assert "flex-web-service" in sent

    def test_missing_row_is_none(self, monkeypatch: pytest.MonkeyPatch):
        from db.hrana_http import read_service_health_http
        ...
        monkeypatch.setattr(
            urllib.request, "urlopen", lambda req, timeout=None: _FakeHttpResponse(empty)
        )

        assert read_service_health_http("flex-web-service") is None

    def test_transport_failure_raises_hrana_error(self, monkeypatch: pytest.MonkeyPatch):
        from db.hrana_http import read_service_health_http
        ...
        with pytest.raises(HranaHttpError):
            read_service_health_http("flex-web-service")
