...
if [[ ! -f "$SRC_REPO/scripts/testing_weekend.sh" ]] \
...
scope_clone_credentials() {
  ...
  [[ -f "$dst" ]] || return 0
  # mktemp creates exclusively and mv renames over dst, so a link planted
  # in the clone after the refusal below is replaced, never written through.
...
if [[ "$SRC_REPO" == "$WEEKEND_REPO" ]]; then
  ...
  # The clone is writable by the sandboxed loop: a planted web or web/.env
  # link would make both functions below write outside it.
