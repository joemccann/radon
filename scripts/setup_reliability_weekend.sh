...
HOST_GITDIR="$WEEKEND_ROOT/.gitdirs/reliability.git"
...
if [[ ! -f "$SRC_REPO/scripts/reliability_weekend.sh" ]] \
...
source "$SRC_REPO/scripts/reliability_weekend.sh" --lock-lib-only
...
touch "$WEEKEND_REPO/.radon-reliability-runner"  # REL-180 (R-504): this loop's own marker
...
# REL-207 (R-569): .mcp.json's radon-kb server execs a RELATIVE
# .venv/bin/python; without this link every claude boot in the runner clone
# fails the MCP with ENOENT.
ln -sfn "$WEEKEND_VENV" "$WEEKEND_REPO/.venv"
( cd "$WEEKEND_REPO" \
