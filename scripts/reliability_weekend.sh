...
# every entry point (and continuation round) runs `git --git-dir="$HOST_GITDIR" --work-tree="$REPO" clean -fdxq`, so a
...
begin_phase() {
  ...
  # Audit gets continuation rounds too, not just remediation. 2026-09-26: the
  # audit declared INCOMPLETE 2 minutes into its 2h cap -- a serial review of a
  # 54-commit delta that the rung gave up on -- and MAX_ROUNDS=1 ended the
  # phase there, leaving 98% of the cap and the whole cycle budget unused while
  # the checkpoint stayed put. Three rounds, bounded by the same per-round cap
  # and the same cycle-deadline check that guards remediation's rounds.
  case "$PHASE" in
    remediate) MAX_ROUNDS=8 ;;
    audit) MAX_ROUNDS="${RADON_WEEKEND_AUDIT_MAX_ROUNDS:-3}" ;;
    *) MAX_ROUNDS=1 ;;
  esac
...
reground_for_continuation() {
  ...
  git --git-dir="$HOST_GITDIR" --work-tree="$REPO" checkout -f --quiet main \
    ...
    && git --git-dir="$HOST_GITDIR" --work-tree="$REPO" clean -fdxq --exclude=.radon-weekend-runner --exclude=.radon-reliability-runner --exclude=.weekend-runner.lock --exclude=logs/ --exclude=.env --exclude=.env.ib-mode --exclude=web/.env --exclude=node_modules/ --exclude=.next/ --exclude=.deepsec/ \
    && align_agent_gitdir
}
...
run_phase() {
  ...
  refuse_billing_reroute_files
  start_browser_host
  echo "[weekend] $PHASE start $STAMP repo=$REPO cap=${CAP_SECS}s${BILLING_IGNORED:+ ignored=${BILLING_IGNORED// /,}}${FOREIGN_LOCK:+ foreign-lock=$FOREIGN_LOCK}${BROWSER_HOST_STATUS:+ browser-host=$BROWSER_HOST_STATUS}" | tee -a "$RUN_LOG"
