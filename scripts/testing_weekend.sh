...
# `remediate` / `deliver`). See .claude/skills/testing-weekend/.
#
...
if [[ -z "$TIMEOUT_BIN" ]]; then
  echo "testing_weekend: GNU timeout (or gtimeout from coreutils) is required" >&2
...
LOOP_SKILL="testing-weekend"
LOOP_LOG_TAG="testing-weekend"
...
if [[ "$MODE" == "cycle" ]]; then
  ...
  RC_DELIVER=$RC
  set -e
  echo "[testing-weekend] cycle done audit_rc=$RC_AUDIT remediate_rc=$RC_REMEDIATE deliver_rc=$RC_DELIVER"
  prune_weekend_root
