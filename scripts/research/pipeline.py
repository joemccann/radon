...
def validate_candidate(value, page_count, folder_date):
    ...
    if not isinstance(value.get('publisher'), str) or not 1 <= len(value['publisher'].strip()) <= 120:
        raise EvidenceError('Invalid publisher')
    try:
    ...
    for figure in figures:
        ...
        if not isinstance(figure.get('caption'), str) or not figure['caption'].strip():
            figure['caption'] = f'Chart, page {figure["page"]}'
    tags = value.get('tags')
...
def date_evidence_passed(candidate, result, page_text):
    """Necessary literal date grounding; reviewer separately verifies date role.

    A complete aggregator byline date is valid. This does not infer dates from
    coverage periods, event calendars, filenames, folder dates or copyright years.
    """
    evidence = result.get('date_evidence') if isinstance(result, dict) else None
    if not isinstance(evidence, dict) or evidence.get('role_verified') is not True or evidence.get('role') not in DATE_EVIDENCE_ROLES:
        return False
        ...
        for start in range(1, count + 1, 8):
            ...
            for candidate in result['candidates']:
                ...
                if (type(date_page) is int and date_page in text
                        and date_page not in candidate['pages']):
