...
[[ -n "$TIMEOUT_BIN" ]] || { echo "security_deepsec_nightly: GNU timeout (or gtimeout from coreutils) is required" >&2; return 78 2>/dev/null || exit 78; }
  ...
  case "$RUNG_PROVIDER" in
    ...
    codex)
      ...
      "$TIMEOUT_BIN" -k "$KILL_AFTER_SECS" "$remain" \
        ...
        -c "sandbox_workspace_write={network_access=true,writable_roots=[\"$AGENT_GITDIR\",\"$WEEKEND_ROOT/.$LOOP_SLUG-deliver\",\"$PRIVATE_SCRATCH\",\"$REPO/.codex/skills\"]}" \
        -C "$REPO" --color never \
...
# This loop is claude-exclusive and stays that way. It is the one loop whose
# findings are sanitized before they reach a public issue, and a fallback CLI
# running the portable prompt cannot be held to that contract. An operator
# ladder naming any other provider is refused, not silently honoured.
refuse_non_claude_rung() {
  local rung
  for rung in "${PROVIDER_RUNGS[@]}"; do
    case "$rung" in
      claude:*) ;;
      *)
        echo "REFUSING: $LOOP_LOG_TAG is claude-exclusive; rung '$rung' names another provider" >&2
        exit 2
        ;;
    esac
  done
...
# `use_rung` above carries that export now; it is set from the rung in force
# and re-exported after every advance.
...
run_phase() {
  begin_phase "$1"
  trap on_crash ERR
  echo "[security-deepsec] $PHASE start $STAMP repo=$REPO cap=${CAP_SECS}s${BILLING_IGNORED:+ ignored=${BILLING_IGNORED// /,}}${FOREIGN_LOCK:+ foreign-lock=$FOREIGN_LOCK}" | tee -a "$RUN_LOG"
  arm_deliver_record
