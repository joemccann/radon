...
# `remediate`). See .claude/skills/documentation-nightly/.
#
...
MODE="${1:?usage: documentation_nightly.sh audit|remediate|cycle}"
...
run_phase() {
  begin_phase "$1"
  trap on_crash ERR
  echo "[documentation-nightly] $PHASE start $STAMP repo=$REPO cap=${CAP_SECS}s" | tee -a "$RUN_LOG"
