...
HOST_GITDIR="$WEEKEND_ROOT/.gitdirs/ci-performance.git"
...
if [[ ! -f "$SRC_REPO/scripts/ci_performance_nightly.sh" ]] \
...
source "$SRC_REPO/scripts/ci_performance_nightly.sh" --lock-lib-only
...
touch "$WEEKEND_REPO/.radon-ci-performance-runner"  # REL-180 (R-504): this loop's own marker
...
SCOPED_ENV="$WEEKEND_ROOT/.env.ci-performance-scoped"
