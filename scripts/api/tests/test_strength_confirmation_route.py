...
def test_strength_confirmation_ticker_scan_bypasses_preset_cooldown(monkeypatch):
    ...
    monkeypatch.setattr(server, "_strength_last_scan", time.monotonic())
    ...
    response = client.post("/strength-confirmation/scan?ticker=mu")
...
def test_strength_confirmation_preset_scan_ignores_explicit_ticker_cache(monkeypatch):
    ...
    client = TestClient(server.app)
    response = client.post("/strength-confirmation/scan?preset=ndx100")
...
def test_strength_confirmation_rejects_invalid_ticker(monkeypatch):
    ...
    client = TestClient(server.app)
    response = client.post("/strength-confirmation/scan?ticker=MU1")
