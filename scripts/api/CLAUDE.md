...
- **Local/tailnet/private-net bypass:** `is_trusted_local_request(request)` = trusted peer **AND** not forwarded. Trusted peers are loopback, the tailnet (`100.64.0.0/10`), and the Hetzner Cloud Network `radon-private` `10.0.0.0/16` (app `10.0.0.2`, broker `10.0.0.4`; NIC attachment is the authenticated channel, same class as the tailnet). Not wider: `docker0` `172.17.0.0/16` and every other RFC1918 range stay untrusted (`test_auth.py` pins `172.17.0.1` false). A trusted-peer request that carries `X-Forwarded-For` / `Forwarded` / `X-Real-IP` / `X-Forwarded-Host` is denied the bypass. A request carrying reverse-proxy headers (Caddy sets `X-Forwarded-For`) is NOT trusted even from 127.0.0.1, so the public `handle_path /api/ib/*` route can't inherit the bypass. Both the middleware and `verify_clerk_jwt` use it. `scripts/api/auth.py`. See `feedback_health_endpoint_public_leak_and_trust_chokepoint.md`.
- **Auth-exempt unconditionally:** `/health`, `/ws-ticket/validate`, `/demo/trial-expiry`, all `*/share` routes. **`/docs` + `/openapi.json` are NOT exempt** (changed 2026-06-28 security audit): they were leaking the full endpoint map to anonymous callers via Caddy `/api/ib/*`. They now require the trusted-local bypass (loopback/tailnet/`10.0.0.0/16` — open Swagger over an SSH tunnel); a public caller gets 401. `AUTH_EXEMPT_PATHS` is pinned by `test_health_payload.py` + `test_route_authz_matrix.py` (full set-equality — adding/removing needs both pins updated). **`/health` is trust-scoped:** untrusted (public/proxied) callers get `{"status":"ok"}` only — never account IDs / IB state / topology — and short-circuit before `check_ib_gateway()`. Trusted local/tailnet/`10.0.0.0/16` callers get the full payload.
