...
- **`docker`** (default local; also Hetzner since 2026-05-07): `ghcr.io/gnzsnz/ib-gateway`, `restart: "no"`. The lock-aware watchdog/FastAPI control plane owns starts. `npm run ib:start`. **Hetzner gotcha:** container launched by `radon-ib-gateway.service` from `/home/radon/radon-cloud/`, not in-tree `<repo>/docker/ib-gateway/`. `IB_GATEWAY_COMPOSE_DIR=/home/radon/radon-cloud` is required.
