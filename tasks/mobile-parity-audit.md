...
- Mobile sort chip parity: CTA chips now cover all 7 desktop CtaSortKeys, discover all 10 DiscoverSortKeys, journal gains Ticker/Cost/Risk (id/structure/decision/qty deliberately omitted as weak card sorts), orders gains a sortbar (Default/Symbol/Qty/Limit/Status; Default keeps original IB order). Chip sets exported and pinned against desktop unions in `tests/mobile-sort-parity.test.tsx` (7 tests). NOTE: `mobile-blotter.spec.ts` "Cost" expectation fails at HEAD (pre-existing, e026b69a journal change) — not from this loop.
- L2 book montage legibility: `.cockpit--mobile` bumps colheads 9→10px, rows 12→13px (+2px row padding), MKT 11→12px, note 10→11px; desktop density unchanged. Pinned by `tests/mobile-book-legibility.test.ts` (montage needs live IB depth, so source-contract pin; eyeball check next RTH depth session).
- /workflow React Flow canvas: VERIFIED at 393×852, no fix needed — palette tap adds a node, canvas drag moves it (>30px displacement asserted), no horizontal page overflow. Pinned in `e2e/mobile-p2-polish.spec.ts`. Nuance: drag asserted via pointer events (React Flow normalizes touch/mouse to pointers); literal finger-drag untested since Playwright touchscreen only taps.

### Deferred P2
(empty — all items resolved 2026-07-03)
