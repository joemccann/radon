...
| **themarketear newsfeed** | dev laptop iteration via `--once` | **Hetzner-canonical** as `radon-newsfeed.service` since 2026-05-03 |
...
**Trades — single source of truth (shipped `6c6f90f`)**: the `journal` table is canonical. Both `/journal` and `/orders` derive from it. `scripts/journal_rehydrate.py` (commit `bbc776e`) persists `realized_pnl` / `cost_basis` / `proceeds` / `realized_quantity` for round-trips (stocks + options) on every fresh rehydrate. `monitor_daemon/handlers/journal_sync` dual-writes live fills every 5 min in market hours. `/orders` uses `web/lib/blotter/fromJournal.ts:journalRowsToBlotter()` with a **union+preference fallback** to `data/blotter.json` for legacy aggregate-only rows that lack explicit P&L (rows pre-date `bbc776e`). When IB Flex Query 1442520 cooldown clears and rehydrate runs, journal rows gain the fields and the fallback decays to redundant — no code change required.

**Media host**: `https://media.radon.run` (Caddy on Hetzner). Newsfeed scraper writes images to `RADON_MEDIA_REMOTE`. On Hetzner that's a local fs path (`/home/radon/radon-cloud/media/`) — rsync skips SSH self-loop. On the laptop dev path it's `radon@ib-gateway:/home/radon/radon-cloud/media/` (Tailscale) or `radon@5.78.148.38:/...` (public IP fallback). DB stores absolute URLs so both peers resolve identically.

**Newsfeed authentication**: headless Playwright in `scripts/newsfeed/browser.js` + `auth.js`. Storage state at `data/newsfeed-storage.json` (gitignored) caches the FirebaseUI session for ~30 days; full re-auth runs every ~6h. Required env: `THEMARKETEAR_EMAIL`, `THEMARKETEAR_PASSWORD` (root `.env` for laptop, `/home/radon/radon-cloud/.env` for Hetzner). One-shot test: `node scripts/newsfeed/index.js --once`. Hetzner cutover (2026-05-03) installed `libnspr4 libnss3 libasound2t64 libatk1.0-0 libatk-bridge2.0-0 libcups2 libdrm2 libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 libgbm1 libpango-1.0-0 libcairo2` once via `sudo npx playwright install-deps chromium`. `deploy.sh` runs `npx playwright install chromium` after npm install. Service: `radon-newsfeed.service` (`Restart=on-failure`, `RestartSec=30`); steady-state cycle ~4s.
