...
- Push to `main` runs CI then automatically starts the Production-environment deploy job after every required test gate passes; no manual approval is configured. Verify with `gh run list --workflow=ci.yml --limit 1`. Monorepo cutover state and env contract: `docs/monorepo-cloud-migration.md`. Live reliability lessons: `tasks/lessons.md` (2026-07-11).
- Local HTML reports that embed images must inline images as base64 `data:` URIs; Chrome blocks `file://` image loading.

## Cursor Cloud specific instructions

Runtimes are preinstalled in the VM snapshot: `bun` 1.3.14 (on PATH via `/usr/local/bin`), `python3.13` (deadsnakes), and the Playwright Chromium browser + system deps. The startup update script only refreshes deps (`pip install -r requirements.txt -r scripts/requirements-api.txt` + `pytest pytest-asyncio pytest-cov` + `cloud/requirements-test.txt`; `bun install` at root and `web`). Standard lint/test/build/run commands live in `web/package.json`, `README.md` "Tests", and `.github/workflows/ci.yml` — use those; notes below are only the non-obvious cloud caveats.

- Dev env files are gitignored and pre-created in the snapshot: root `.env` and `web/.env` with dev-only values (dummy Clerk keys, `RADON_DB_NO_REPLICA=1`). No live broker/data credentials are present, so IB Gateway / Turso / Unusual Whales features degrade gracefully (empty states, "RELAY OFFLINE"); this is expected, not a bug. If they go missing, recreate them from `.env.example` / `web/.env.example`.
- `IB_GATEWAY_MODE=cloud` is REQUIRED locally here. FastAPI's startup `ensure_ib_gateway()` runs `docker compose` under the default `docker` mode; Docker is not installed, so `docker` mode crashes the FastAPI boot with `FileNotFoundError`. `cloud` mode does a TCP probe only and boots fine (IB pool connect failures to `127.0.0.1:4001` are logged and expected).
- There is NO plain localhost auth bypass — the middleware bypass is token-gated (`web/middleware.ts`). To browse the app without a Clerk session, the server needs `RADON_AUTHLESS_TEST=1` + `RADON_AUTHLESS_TEST_TOKEN` (set in `web/.env`) AND each request must carry header `x-radon-authless-test: <token>`. Curl/Playwright can send it (`web/playwright.config.ts` does automatically); a plain browser tab cannot without a proxy/extension.
- Run services (do NOT use the full `bun run dev` — the IB relay + newsfeed scraper spam connect errors without a gateway/creds): FastAPI `python3.13 -m uvicorn scripts.api.server:app --host 127.0.0.1 --port 8321 --reload --app-dir . --reload-dir scripts` (from repo root) and Next.js `bun run dev:next` (from `web/`).
- Full-suite `next dev` first-hit route compiles are heavy; give first navigations a long timeout. `next build` uses compile mode (`--experimental-build-mode=compile`) and needs dummy Clerk env (`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`) to complete.
- The one-off dir `/var/lib/radon` (2FA lock / health state) is created writable in the snapshot; FastAPI logs a `PermissionError` for it if absent (non-fatal).
- Known pre-existing test fragility: `web/tests/previous-close-yahoo-daily-array.test.ts` builds timestamps from `Date.now()` and asserts a weekday session index, so it fails when run on a weekend (not an environment issue).
