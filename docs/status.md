...
- 2026-03-19 — **feat: Portfolio P&L overhaul — 13 commits.** Same-day P&L uses entry cost baseline (not yesterday's close). Stale option prices use bid/ask mid when last outside spread. Share PnL card: correct Return on Risk % (P&L / entry cost), derived entry price for fully-closed positions, entry timestamp from trade_log_dates API. Executed order net price shown for single-leg fills. Combo classification (bear put spread vs risk reversal). Table search/filter on all 5 tables. `entry_date` derived from trade_log (ticker+structure match). `usePortfolio` backoff resets on tab visibility. 1316 tests.
- 2026-03-19 — **fix: Preserve real `/orders` cancel failures end to end.** IB cancel confirmation now re-checks refreshed open orders and treats disappearance as success; FastAPI extracts human-readable JSON script errors; Next `/api/orders/cancel` preserves upstream status/detail instead of collapsing upstream `502`s to a generic `500`. Added Python, route, Playwright, and chrome-cdp verification.
