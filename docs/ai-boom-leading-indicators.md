...
| Public US GPU index ticker (A100/H100/H200/B200/B300) | Liquid Compute | https://liquidcompute.com · `GET https://liquidcompute.com/api/market/ticker` | **D** homepage `asOf` | Same day / T+1 | Homepage-public payload. robots.txt Disallows `/api/` but the public homepage loads this ticker. Identify Radon; do not scrape `/auth/` or `/ingest/`. | **Third venue versus the rental book.** Host-tagged `liquidcompute` / compute `C5`. Methodology opaque until licensed. Never splice onto gpurentalprices, Silicon Data, or C1 asking prices. Sample ingest 2026-09-14: H100-US 2.6766 usd_per_gpu_per_hr. |
