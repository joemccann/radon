...
worst outcome, REL-210). Transient sFTP GET failures (`kex_exchange_identification`, connection reset, connection timed out; `_is_transient_sftp_get`) record `class=sftp_transient` and do not fail the run only when a strictly newer statement from the same account and query was applied or confirmed duplicate in this sweep. Missing or unparseable delivery dates are not suppressed. A reset on a different query is not covered. Host-key, decrypt, and ingest rejects remain fail-closed. Failures name the affected query/account keys in the final error heartbeat, including on repeated runs. A TWR build that returns `degraded` / `unavailable` after the cash-flow persist is not an ingest failure (perf-twr owns that page), page 5a2eb828, 2026-09-16. After the ingest, `heal_twr_coverage_gaps` replays delivered Activity statements for stored NAV sessions with no `twr_subperiods` row (`TWR_GAP_LOOKBACK` 45 days before the newest statement), oldest-first through `build_and_persist(from_file=...)` only, never the cash or journal writers, inside the same sweep budget. The `ok` row notes `twr_gap_healed`, `twr_gap_unhealed`, or `twr_gap_unhealable` (no delivered statement; that session and later ones stay unchained, never zero-filled). Runbook `flex-pull-twr-gap`. Stripped env
...
A duplicate Flex ingest verifies cash-flow IDs and NAV dates, or journal execution coverage, through bounded Hrana reads before reporting persistence. An applied activity duplicate with all expected cash-flow IDs present can insert missing NAV dates from the saved statement; this retry is not read-only. It never overwrites existing NAV corrections and never replays cash. See the [activity NAV incident diagnosis](incident-runbook.md#flex-pull-activity-nav) and the [ingest implementation](../scripts/flex_delivery_ingest.py). Flex trade IDs that were not stored because individual IB fills already match that contract-day's quantity and gross notional count as covered. A quantity or notional disagreement does not. Missing cash, unreadable coverage, or coverage still unverified after the NAV repair retains the applied claim, returns an error, and requires operator reconciliation; the puller rejects duplicate results without explicit confirmation. It never replays an applied delivery.
