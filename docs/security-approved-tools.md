...
|---|---|---|---|
| Claude Code (`claude --version`) | 2.1.278 | 2026-09-19 | Automated: `scripts/claude_cli_env_drift.py` (launchd `com.radon.claude-cli-env-drift`, daily 23:50, install `bash scripts/setup_claude_cli_env_drift.sh`) pages Pushover once per CLI version when the binary references a name missing from `scripts/claude_cli_env_reviewed.txt`. On a page, re-derive the billing-reroute lists: `strings -n 8 ~/.local/share/claude/versions/<ver> \| grep -o -E '(ANTHROPIC_\|CLAUDE_CODE_\|AWS_BEARER_)[A-Z0-9_]+' \| sort -u`, read each new name in context, add any that reroutes model auth or billing to `BILLING_REROUTE_KEYS` / `BILLING_REROUTE_FLAGS` in every wrapper, and re-run `scripts/tests/test_weekend_subscription_only.py`, then append every reviewed name to `scripts/claude_cli_env_reviewed.txt`. |
