...
| deepsec (`vercel-labs/deepsec`) | 2.3.8 | 2026-09-14 | Sibling worker `scripts/security_deepsec_worker.sh` writes `deepsec-version.log` per run; the ignored `.deepsec/` workspace must pin the same version. Lockfile review is still OPERATOR_REQUIRED (see the skill, Stage 3). |
