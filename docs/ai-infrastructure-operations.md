...
The `/regime/llm` AI infrastructure view reads the compact `ai_cycle_api_snapshot` row written by collection. Browser requests never scrape providers, scan observation history, or place trades. Rebuild that snapshot with `python3.13 -m scripts.ai_cycle --persist-snapshot`. All automatic source collection runs through `python3.13 -m scripts.ai_cycle` from the repository root. `radon-ai-cycle-backfill.timer` resumes historical coverage at 05:30 UTC, `radon-aa-frontier-refresh.timer` refreshes the model cohort at 07:00 UTC, then `radon-ai-cycle.timer` collects current observations at 07:15 UTC. `radon-liquidcompute.timer` polls the Liquid Compute homepage GPU index at 07:30 UTC (`python3.13 -m scripts.ai_cycle.liquidcompute --record`). Enable the dedicated job with `systemctl enable --now radon-liquidcompute.timer`. The ticker is also a default `scripts.ai_cycle` source so `--record` without `--sources` persists it. Rows land in host-tagged `liquidcompute_index` (`source=liquidcompute`, `date`/`vintage` from publisher `asOf`) and as compute `C5`. Never splice onto GPU Rental Prices, Silicon Data, or C1 asking-price cohorts.
...
| OpenDesign Arena | current snapshot only | [LLM Arena for Design](https://open-design.ai/llm-arena-for-design/) | Public HTML. Demand `D6` LLM/model-quality stub: score, cost per artifact and minutes by model and task family. Default and scheduled collection fetch the live page; transport or markup failures never replay the bundled test fixture. Offline imports require `--import-opendesi <html>` plus `--opendesi-captured-at <ISO timestamp with timezone>`; observation period/asof retain that capture time while fetched_at records import time. Broken markup records an honest error and invents no scores. Never mixed into GPU scarcity / Silicon Data. |
| Liquid Compute | current snapshot only | [Homepage GPU index ticker](https://liquidcompute.com/api/market/ticker) | Public homepage payload (`asOf`, `usd_per_gpu_per_hr`, index ids such as `h100-us`). robots.txt Disallows `/api/` but the public homepage loads this ticker; treat as homepage data, identify Radon, back off on non-200, and do not scrape `/auth/` or `/ingest/`. Host-tagged store is idempotent on `(source, series_id, asOf)`. Methodology opaque until licensed. Third venue versus the rental book; never spliced onto gpurentalprices or Silicon Data. |
...
- Liquid Compute homepage ticker returned HTTP 200 on 2026-09-15 (`asOf` 2026-09-14, five US index ids). Stored host-tagged as `liquidcompute` / C5. Methodology is opaque until licensed. Never mixed into GPU scarcity / Silicon Data / gpurentalprices.
