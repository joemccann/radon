...
| OpenDesign Arena | current snapshot only | [LLM Arena for Design](https://open-design.ai/llm-arena-for-design/) | Public HTML. Demand `D6` LLM/model-quality stub: score, cost per artifact and minutes by model and task family. Default collect uses the captured fixture; `--live-opendesi` fetches the page. Broken markup records an honest error and invents no scores. Never mixed into GPU scarcity / Silicon Data. |
