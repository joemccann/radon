...
| **CTA vision model ladder** | Extracts MenthorQ CTA screenshots when HTML data is unavailable. Configure one or more optional provider credentials in [`cloud/.env.example`](../cloud/.env.example); the source-owned order is in [`scripts/clients/model_ladder.py`](../scripts/clients/model_ladder.py). A provider billing, quota, or hard failure tries the next configured provider; Cursor has no vision HTTP path. No configured provider, or an exhausted configured ladder, fails the CTA extraction and reports `vision_cascade_exhausted`; set a remaining provider credential and verify the next `radon-cta-sync` run reports a successful provider before treating CTA data as current. Do not add these fallback credentials to `cloud/config/required-env.txt`. | Optional provider keys in [`cloud/.env.example`](../cloud/.env.example) | Provider consoles |
...
| **Model ladder (text JSON)** | Newsfeed text tagging and knowledge distillation. Shared order in [`scripts/clients/model_ladder.py`](../scripts/clients/model_ladder.py): subscription `anthropic -> grok -> cursor -> codex -> gemini`, then `nvidia`, then `cerebras` last. JS tagger reaches it via `scripts/clients/model_ladder_cli.py`. Soft-fails when no keyed provider works. | Optional provider keys in [`.env.example`](../.env.example) | Provider consoles |
...
`/regime/llm` now groups Demand, Compute, Delivery and Finance. The collector uses OpenRouter, Vercel AI Gateway, GPU Rental Prices, Artificial Analysis, SEC, EIA, Vast, OpenDesign Arena (Demand `D6`, LLM/model-quality only; never GPU scarcity) and Liquid Compute (Compute `C5`, homepage GPU index ticker; third venue versus the rental book, methodology opaque until licensed) for their specific published measurements. Missing credentials and unsupported feeds remain visible source states. Issuer disclosures require reviewed imports; no automatic filing-text extraction is claimed. See [source configuration and limits](ai-infrastructure-operations.md) and [primary-statement reconciliation](ai-infrastructure-verification.md).
