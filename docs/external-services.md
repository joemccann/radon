...
`/regime/llm` now groups Demand, Compute, Delivery and Finance. The collector uses OpenRouter, Vercel AI Gateway, GPU Rental Prices, Artificial Analysis, SEC, EIA, Vast and OpenDesign Arena (Demand `D6`, LLM/model-quality only; never GPU scarcity) for their specific published measurements. Missing credentials and unsupported feeds remain visible source states. Issuer disclosures require reviewed imports; no automatic filing-text extraction is claimed. See [source configuration and limits](ai-infrastructure-operations.md) and [primary-statement reconciliation](ai-infrastructure-verification.md).
