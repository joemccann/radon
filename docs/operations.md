...
**The ladder crosses providers, and the security loop is claude-exclusive.** A rung is `provider:model`. The security loop's ladder is the four Claude models and nothing else: it is the one loop whose findings are sanitized before they reach a public issue, and its wrapper refuses at startup (exit 2) if `RADON_WEEKEND_PROVIDER_LADDER` names any other provider. The other four left the claude.ai subscription entirely on 2026-09-06 — it is reserved for security — and run `codex:gpt-5.4`, then `grok:grok-4.6`, then `nvidia:nvidia/nemotron-3-ultra-550b-a55b`, then `cerebras:qwen-3.8-27b`. Override with `RADON_WEEKEND_PROVIDER_LADDER` (space-separated `provider:model` rungs); the security loop still also honours `RADON_WEEKEND_MODEL_LADDER` for its Claude rungs. Defaults are pinned by `scripts/tests/test_provider_registry_parity.py`.
