...
**DeepSec is a sibling worker, not a sixth loop.** `com.radon.security-deepsec` (`scripts/security_deepsec_worker.sh`) fires at the same 00:40 slot in `~/radon-weekend/radon-security-deepsec`, with its own lock (`~/radon-weekend/.security-deepsec.lock`), cap (`RADON_WEEKEND_DEEPSEC_CAP_SECS`, default 8h), and dead-man label `security-deepsec`. It writes verified exports into the shared private scratch and does not remediate or deliver. The security audit harvests whatever export is already ready and must not report TIMEOUT solely because DeepSec is still running. Operators read two issues: `security-nightly` for audit/remediate/deliver, `security-deepsec` for still running / failed / export ready / harvested. A `failed` DeepSec status is operator-only: first confirm the jobs with `launchctl list | grep radon`. Do not bootstrap or restart DeepSec from a nightly run. On the Mini, repair the official pinned workspace, then run `bash scripts/setup_security_nightly.sh` only when neither the security loop lock nor `~/radon-weekend/.security-deepsec.lock` has a live PID; the setup script refuses those races and reports missing DeepSec as an advisory.

**Codemap refresh (02:00, not an agent loop).** `scripts/codemap_nightly.sh` / `config/com.radon.codemap-nightly.plist` runs in `~/radon-weekend/radon-codemap`: regenerates `tools/codemap/*` from `origin/main`, verifies the regenerated artifacts against the live import graph before opening a `codemap/<date>` PR, waits for required checks, then squash-merges. Feature branches never commit those artifacts. Install: `bash scripts/setup_codemap_nightly.sh`.
...
phase is scored on a commit to that branch. Arming the deliver record at
