...
**Codemap refresh (02:00, not an agent loop).** `scripts/codemap_nightly.sh` / `config/com.radon.codemap-nightly.plist` runs in `~/radon-weekend/radon-codemap`: regenerates `tools/codemap/*` from `origin/main`, opens a `codemap/<date>` PR, waits for required checks, squash-merges. Feature branches never commit those artifacts. Install: `bash scripts/setup_codemap_nightly.sh`.
