...
Stable Claude Code path for macOS privacy grants (2026-09-23). The native installer gives every release a new path, `~/.local/share/claude/versions/<version>`. That binary is a bare Mach-O, so TCC records Full Disk Access and folder grants against its path, and System Settings grew one row per CLI version while the loops prompted again after each update. The six loop plists now put `~/.local/share/radon/claude-stable` ahead of `~/.local/bin`, so `command -v claude` resolves to one fixed-path copy. `com.radon.claude-stable-sync` (installed by `bash scripts/setup_claude_stable_sync.sh`) runs `scripts/claude_stable_sync.sh` from an installed copy outside every clone. It runs at load, whenever `~/.local/share/claude/versions` or `~/.local/bin` changes, and daily at 23:45. It copies the binary `~/.local/bin/claude` resolves to (an APFS clone, never a hard link), and only after `codesign --verify --strict` passes against Anthropic's designated requirement (`com.anthropic.claude-code`, team `Q6L2SF6YDW`). Anything else is `REFUSED` and the copy is left unchanged. Grant Full Disk Access once to `~/.local/share/radon/claude-stable/claude`. The grant survives updates because TCC checks the stored designated requirement, not the hash. If the copy is missing, PATH falls through to `~/.local/bin/claude` as before.

Stable fx path (2026-09-26). The same pattern for Vercel fx: `fx upgrade` replaces `~/.local/bin/fx`, so the operator re-added fx to Full Disk Access after upgrades. `com.radon.fx-stable-sync` (installed by `bash scripts/setup_fx_stable_sync.sh`) runs `scripts/fx_stable_sync.sh` from an installed copy at load, whenever `~/.local/bin` changes, and daily at 23:45. It copies `~/.local/bin/fx` to `~/.local/share/radon/fx-stable/fx` (APFS clone) only after `codesign --verify --strict` passes against Vercel's designated requirement (`com.vercel.fx`, team `JW6Y669B67`). Every loop wrapper's `provider_bin fx` prefers that copy, then `~/.local/bin/fx`; `RADON_WEEKEND_FX_BIN` still overrides both. Grant Full Disk Access once to `~/.local/share/radon/fx-stable/fx`.

Scheduled browser and lock policy (2026-09-22). Testing and reliability no longer start a shared host Playwright server. Every scheduled provider receives `RADON_WEEKEND_BROWSER_HOST=unavailable:disabled`, and inherited `PW_TEST_CONNECT_WS_ENDPOINT` is removed before each round. Sandboxed UI checks and screenshots run in GitHub CI; unsandboxed providers may use local Playwright. Setup's browser installation smoke is temporary and does not enable a scheduled server. Reinstall only while every runner is idle. Host git for every loop uses `$WEEKEND_ROOT/.gitdirs/<loop>.git` with `--work-tree` on the clone, not the clone `.git`. Codex sandbox writable roots omit that host gitdir. They do include the clone's tracked `.codex/skills` renders, which the seatbelt otherwise keeps read-only, so a checkout between commits whose renders differ cannot half-apply (`.codex/config.toml` stays read-only). Codex runs commands in a non-login shell (`allow_login_shell=false`) so the operator profile cannot put Homebrew's `python3.13` ahead of the loop venv (2026-09-25). Split gitdirs (2026-09-23): the clone `.git` gitfile names the rung's own gitdir, `$WEEKEND_ROOT/.gitdirs-agent/<loop>.git`, which is in the codex writable roots, so every rung can branch, commit, fetch, `merge-tree` and push. Host git never opens it. Before every round the wrapper rewrites its config (core settings plus `remote.origin`), object alternates (the host gitdir's objects) and the gitfile from host state, and drops `hooks/` and `commondir`. After each host reset it also moves the agent's `main`, `origin/main`, `HEAD` and index to the tree the host checked out. Dated branches survive across phases; one with no commits past `main` (a strict ancestor of host `HEAD`) is moved to `HEAD`, so a later round never resumes a stale tree (2026-09-24). A branch holding its own commits is never moved. Commit evidence reads the agent HEAD from plain files through a throwaway host-written gitdir. `nightly_green_base.py` runs with `GIT_DIR` pinned to the host gitdir, and `weekend_prune.py` does not touch worktrees of a gitfile clone. Launchd exits 0 without launching a wrapper when a runner lock is live (a live pid whose start time still matches the lock, so a reused pid no longer skips the night), and exits 70 if fetch (three attempts), checkout, or reset fails; both page Pushover from the shared `.env`, never silently. The launchd pre-lock fetch is bounded by `timeout -k 15 180`, falling back to `gtimeout` (macOS coreutils); a host with neither exits 78 before any git runs, so an unbounded fetch can never hold the slot (DS-2026-09-23-07). A plist change only takes effect once each `setup_*` script re-renders the installed copy in `~/Library/LaunchAgents`. On the VPS, `setup-vps.sh` creates `/home/radon/.radon-deploy.lock` with `install -T`, so a link raced in after its checks is replaced, never entered. Wrapper ground-truth refresh propagates these failures even when called from a Bash conditional. Security setup reruns preserve the existing DeepSec separate gitdir; only legacy linked worktrees without that gitdir are migrated. A valid runner lock is only the per-clone directory `$REPO/.weekend-runner.lock/` with `pid` then `start` (`ps -o lstart=`). `~/radon-weekend/.weekend-runner.lock` as a plain file is never valid: every wrapper and `setup_*` sweeps it to `~/radon-weekend/.stale-locks/`. Host bash reclaims a lock when `pid_alive` proves death (ESRCH or start-fingerprint mismatch). An empty/unpublished pid is in flight for its first five minutes (`RADON_WEEKEND_LOCK_GRACE_MINS`) and abandoned after. `EPERM` / `PermissionError` is not death. Sandboxed agents must never `kill -0`, reclaim, or create a runner lock.

Runner lifecycle hardening (2026-09-24, `scripts/tests/test_loop_lifecycle_adversarial.py`). The runner lock names the wrapper, but the work is the agent's process tree, and several failures came from the two getting out of step. Each item below was reproduced against the prior code before it was fixed:
- **Detached agent children.** An agent child that detached into its own session outlived its round: the testing skill starts pytest with `start_new_session=True`, and on 09-22 and 09-23 it kept writing into the testing clone through the next phase's `git clean`. `kill_round_group` now sends TERM to the round's process group, waits 10 seconds (3 on the signal path), then sends KILL.

Round membership is never guessed from a process's attributes. On the runner, `~/radon-weekend/radon` doubles as the Remote Control workspace: `radon-rc.sh` agents, Claude Code daemons, interactive sessions and whatever they background all sit in it with no tty, often orphaned. That breaks the documented "never point another job at these clones" rule, and they should move out of the clone.

Beyond the group, a process is reaped only when it is positively known to be the round's:
- **Sampled:** a sampler records the round's descendants (pid plus start time) every second, and the process still carries its sampled start time.
- **Declared:** the agent listed its pid, one per line, in `$RADON_WEEKEND_DETACHED_PIDFILE` (`$REPO/.weekend-detached-pids`) within 30 seconds of starting it. It must also work in the clone, have started during the round, have no tty, and neither it nor any ancestor may be a Claude Code daemon or `radon-rc.sh`. The sampler copies a valid declaration, with its start time, into the round's record. A reap never re-reads the agent-writable file, so a stale or sloppy entry (`pgrep -f pytest`) or a reused pid cannot match.

The live descendants of a reaped process go with it, except Remote Control and daemon processes and anything under them. The record is rebuilt each second as the live set, from one `ps` snapshot, so it stays bounded and scanning it costs one `ps` call. The sampler stops if the round's pid is reused. The records live outside every clone, in `$WEEKEND_ROOT/.runner-state/<clone>/`. The testing skill's detached gate script declares its pid, because its launcher exits too fast for any sampler.
- **SIGKILLed wrapper.** A wrapper SIGKILLed mid-round left its agent running under `timeout`, and the next run took the lock and worked the same clone alongside it. Each round's process group and its start time are now recorded in the state directory; the sampler keeps recording even after its wrapper dies. Before it takes over, a reclaim reaps that group, trusted only while its leader keeps the recorded start time and still works inside the clone. It also reaps the dead run's sampled and declared leftovers.
- **Concurrent reclaim.** Concurrent reclaims of one stale lock produced two owners in 7 of 80 trials. Reclaiming now runs under a `$REPO/.weekend-runner.lock.reclaim` guard that records its owner, and re-judges the owner under it. The guard is broken only when that owner is dead, never by age alone, and it is moved aside rather than deleted. After moving a guard or a lock aside, the reclaimer checks by inode that it moved the one it judged, and puts it back if not.
- **Timezone-dependent start time.** The lock's start fingerprint followed the checker's TZ and locale. It is written and compared as `LC_ALL=C TZ=UTC ps -o lstart=`, and the local form is still accepted for older locks.
- **Leftover git locks.** Abandoned host-gitdir git locks are cleared by every wrapper's `ground_truth`, and by launchd before its checkout. Top-level `index.lock` and `HEAD.lock` are removed outright. Ref, `packed-refs`, `shallow` and `config` locks are removed once they are ten minutes old. `worktrees/` and `objects/` are never touched.
- **ExitTimeOut.** The loop plists set `ExitTimeOut` to 60, because launchd's short default SIGKILLed the signal path before it could page.
- **Green-base pin.** `nightly_green_base.py` now filters the `ci.yml` run listing itself. GitHub's search-backed `branch`/`event`/`status` filters intermittently served a listing frozen at 2026-09-19, which pinned eight phases to e1a0237d. A listing that has no run at all for a tip more than an hour old is treated as stale, and the tip is kept. A red tip after a quiet weekend still pins Friday's green, because its failed run is in the listing. The resolver's reasoning goes to the phase run log.
...
**Cycle shape (2026-09-02).** `audit` (cap 2h, `RADON_WEEKEND_AUDIT_CAP_SECS`) records verified findings -- in the reliability loop it also gets up to 3 continuation rounds (`RADON_WEEKEND_AUDIT_MAX_ROUNDS`, default 3) because on 2026-09-26 its audit declared INCOMPLETE two minutes into the cap and the single-round loop ended the phase with the checkpoint unmoved and 98% of the cap unused; rounds stay bounded by the same per-round cap and cycle-deadline check as remediation's 8, a cap-killed audit (124) is NOT relaunched because the cap is the authority on a phase holding no durable partial state, and an audit relaunch is logged rather than reported so one comment plus one Pushover per phase still holds; `remediate` (cap 6h, `RADON_WEEKEND_REMEDIATE_CAP_SECS`) implements EVERY verified source-actionable finding as root-cause commits on one dated branch `<loop>/<YYYY-MM-DD>`; `deliver` (cap 3h, `RADON_WEEKEND_DELIVER_CAP_SECS`) formats ONE substantive PR via `scripts/github_pr_output.py`, publishes through `scripts/nightly_publish.py`, polls CI with `scripts/nightly_deliver.py watch`, fixes red checks on the branch, and ends by printing a verdict line the wrapper turns into the cycle's final notification. Deliver runs even when remediate exited non-zero (committed fixes are durable; CI decides). The loop never merges: the operator merges from the Pushover / issue line. The `gh` shim (`scripts/nightly_pr_guard.py`) refuses a merge, and for the security loops any issue write, wherever the endpoint sits among the `gh api` arguments. An INCOMPLETE deliver records branch + PR number outside the clone (`~/radon-weekend/.<loop>-deliver/record.json`; the security loop also mirrors it in its private run-record) and the next fire resumes that branch and PR before opening a new one.

The testing wrapper retries an audit once when its agent exits successfully without a substantive commit or a completed no-op declaration. The retry shares the original phase deadline; another empty exit remains INCOMPLETE. Failed, timed-out and truncated runs are not eligible for this recovery. Contract: `scripts/tests/test_weekend_loop_deadman.py`.
...
Publication must use `python3.13 scripts/nightly_publish.py publish --base main --head <branch> --title <title> --body-file <path>`. The publisher refreshes the base, classifies the committed changes, owns the push and PR creation/reuse, and reports the actual PR URL and head SHA (exit 0); no-op exits 3 before push or PR creation, and errors exit 1. The agent runners also prepend an `origin/main` snapshot of the `gh` guard to PATH to catch accidental direct PR creation. It classifies argv the way gh does (any non-bool flag consumes the next token, so a flag value is never read as a subcommand or `api` endpoint), refuses merges and `gh alias set` / `gh alias import`, and routes every `pr`, `api`, `issue` and `alias` invocation through it. That shim is best-effort accident prevention, not a security sandbox; the shared publisher is the publication gate. Resume an existing substantive PR before declaring nothing to ship. With none to resume, record `--branch "" --status green` through `scripts/nightly_deliver.py record`, omit PR identifiers and URLs, then print `NIGHTLY DELIVER READY: loop=<slug> prs=0`.
...
**Preflight before a night you need to land.** Run `bash scripts/nightly_smoke.sh` on the runner host; exit 0 means every loop above will start. For each loop it checks that the launchd job is loaded, the runner lock is free, the exact launchd `git fetch` succeeds through the host gitdir, the `origin/main` wrapper parses, a provider-ladder rung is ready (using the wrapper's own `provider_ready`), and the test venv has pytest. It also checks `timeout` on the launchd PATH, that system sleep is off and that no Claude billing reroute is set. It launches no agent and takes no lock. `scripts/tests/test_nightly_smoke.py` fails if a scheduled loop is missing from its table.

**The testing clone uses least-privilege scoped credentials.** `scripts/setup_testing_weekend.sh` copies your `web/.env` into `~/radon-weekend/radon-testing`, then replaces `TURSO_AUTH_TOKEN` and `UW_TOKEN` in that copy with the values from the operator-owned `~/radon-weekend/.env.testing-scoped` (mode 0600, outside every clone): a read-only Turso token (`turso db tokens create radon --read-only`) and the Unusual Whales key. UW issues one key per account (the dashboard offers only Reveal, Copy and Regenerate), so the loops deliberately share the production key and its daily request quota (operator decision, 2026-09-25); never press Regenerate, which revokes the key production uses. A missing file, key or empty value removes that key from the clone copy and prints a `MISSING` line naming the file, never a value. The step reruns on every setup, and `scripts/testing_weekend.sh` never re-copies `web/.env`, so the production values cannot come back between setups. Rotate a scoped key by editing that file and re-running setup. Setup refuses to run when the clone's `web` or `web/.env` is a symlink: delete the link and re-run. Contract: `scripts/tests/test_weekend_runner_env_provisioning.py` (`TestTestingCloneScopedCredentials`). The reliability, ci-performance and documentation clones follow the same rule: `setup_reliability_weekend.sh`, `setup_ci_performance.sh` and `setup_documentation_nightly.sh` scope `TURSO_AUTH_TOKEN` and `UW_TOKEN` in their clone's `web/.env` from `~/radon-weekend/.env.reliability-scoped`, `.env.ci-performance-scoped` and `.env.documentation-scoped` (each mode 0600), with the same strip-on-missing, no-echo and symlink refusal.
...
**The ladder crosses providers, and the security loop is claude-exclusive.** A rung is `provider:model`. Security and DeepSec share `scripts/security_claude_ladder.sh`: at run time they list Mini `claude models` (subscription CLI only; no Anthropic API key path), skip the newest generation, and run the prior / second-newest first, then deeper Claude fallbacks. `--effort medium` is pinned on every Claude launch. `RADON_WEEKEND_MODEL_LADDER` / `RADON_WEEKEND_PROVIDER_LADDER` skip discovery when set. Discovery failure logs and uses the safety ladder `claude-opus-5` then `claude-sonnet-5` (newest / fable excluded). The wrapper refuses at startup (exit 2) if `RADON_WEEKEND_PROVIDER_LADDER` names any other provider. The other four left the claude.ai subscription entirely on 2026-09-06 — it is reserved for security — and, for reliability, testing and ci-performance, run `fx:nvidia`, then `grok`, then `codex`, then `fx:cerebras` (operator's order, 2026-09-26; the fx model half names the fx provider and picks the one key exported, `NVIDIA_API_KEY` or `CEREBRAS_API_KEY`); the documentation loop alone runs one rung, `fx:nvidia` (2026-09-26): Vercel fx (the stable copy above, else `~/.local/bin/fx`, override `RADON_WEEKEND_FX_BIN`) drives NVIDIA NIM over Chat Completions directly, with no codex, grok or cerebras fallback, so a capped or missing fx is an honest INCOMPLETE. `scripts/agent_cli_bootstrap.sh` merges the `nvidia` and `cerebras` providers and resolved models into `~/.fx/settings.json`. fx runs shell commands in the account's login zsh, which would put `/opt/homebrew/bin` ahead of the loop venv, so the wrapper launches it with a runner-owned `ZDOTDIR` (`~/.radon/agent-cli/fx-zdotdir`, rewritten every round) whose `.zshrc`/`.zlogin` restore the wrapper's PATH from `RADON_AGENT_PATH`. Pinned by `scripts/tests/test_fx_nvidia_rung.py`. The model half of a rung is optional as of 2026-09-07: a bare `codex` or `grok` rung launches that CLI with no `--model`, so the account default the vendor migrates forward is what runs (a pinned `gpt-5.4` answered HTTP 400 `invalid_request_error` once OpenAI moved the default to `gpt-5.6-terra`, and three loops died with no issue comment). Every fallback rung runs at medium reasoning effort (`-c model_reasoning_effort="medium"` for codex, `--reasoning-effort medium` for grok). The claude rung is launched with `--disallowedTools ScheduleWakeup Monitor CronCreate` (2026-09-08): three security rounds in a row ran 17-27 minutes, wrote nothing to the run log and exited 0, each transcript ending on `ScheduleWakeup` "while <stage> runs detached; notifications are the primary wake signal" — under `claude -p` there is no later, so the turn ends with the final text and the completion marker never printed. The skill's in-session `until` loop on the rc file is the only sanctioned wait. Pinned by `scripts/tests/test_provider_invocation_contract.py` (`TestTheClaudeWire`). `nvidia` and `cerebras` still name a model because the grok CLI resolves them through a `[model."<key>"]` block, but they name a STABLE KEY only; `scripts/agent_cli_bootstrap.sh` resolves the live id behind it from the provider's `/v1/models` at provision time (nvidia: largest/newest `nvidia/*`, the only NIM models that deserialize cleanly through grok), so a new model release is picked up by re-running that script, never by editing a wrapper. Override with `RADON_WEEKEND_PROVIDER_LADDER` (space-separated `provider[:model]` rungs); the security loop still also honours `RADON_WEEKEND_MODEL_LADDER` for its Claude rungs. Both security runners (`security_nightly.sh`, `security_deepsec_nightly.sh`) export `RADON_LADDER_NO_AUTH_FILES=1` so no child python's ladder auth discovery reads the operator's on-disk subscription grants (2026-09-19). They also `export RADON_REPO_ROOT=$REPO` so the native `.claude/workflows/security-audit*.mjs` workflows resolve the clone from that env, then `RADON_WEEKEND_REPO`, then `git` toplevel, then cwd, instead of a hardcoded laptop path. Defaults are pinned by `scripts/tests/test_provider_registry_parity.py` and `scripts/tests/test_provider_failover.py`.

A rung can fail in four ways, and each is classified separately from this round's log slice: a shared **account cap** retires the whole provider (`advance_rung wide`), a per-model **quota** advances one rung, and a permanent **rejection** (HTTP 400 `invalid_request_error`, "model is not supported", "Model metadata for `…` not found", "unknown model") also advances one rung — another model or provider may be fine — and consumes none of the three transient-network attempts. The fourth is a rung that **crashed inside its own CLI** after answering (`is_rung_broken`: a column-0 `Internal error:` or `serialization error:` line in the last 40 lines of this round's slice, the column anchor being what keeps a wrapper audit quoting the text from tripping it). It also costs one rung and none of the three attempts, because the rung billed tokens and is therefore neither capped nor unreachable — it is simply unable to finish a turn.
...
**A non-Claude CLI cannot resolve a Claude slash command.** codex and grok get `.claude/portable-prompts/<skill>.<phase>.md` instead — the SKILL.md body with its frontmatter stripped, wrapped in a preamble, an OVERRIDES section (no subagents, Playwright instead of chrome-cdp, `RADON_WEEKEND_REDUCED=1` narrows remediation to P0/P1) and a CONTRACT section naming the exact strings the wrapper greps for. The files are committed and rendered by `scripts/render_loop_prompt.py --write`; `scripts/tests/test_portable_prompt_sync.py` fails when one drifts from a fresh render, which is what keeps editing a SKILL.md honest. The security loop has no portable prompt, by design. Provider credentials and the grok-hosted endpoint configs live outside every clone in `~/.radon/agent-cli/` (provisioned by `scripts/agent_cli_bootstrap.sh`), because the loops' credential rails refuse a key file inside the clone and `git clean` between phases would delete one anyway. grok's CLI hosts NVIDIA and Cerebras because it is the only agent CLI here that speaks `/chat/completions`; codex speaks only `/responses`, which neither serves. Only NVIDIA's own `nvidia/*` models are usable through that path — third-party NIM models (kimi-k3, deepseek-v4-pro, minimax-m3) answer correctly and then exit 1 on grok's usage deserializer -- and as of 2026-09-26 `nvidia/nemotron-3-ultra-550b-a55b` fails the same way on any turn that calls a tool, which is every turn one of these loops takes. That is what moved the documentation loop to `fx:nvidia`, which reaches NIM without the grok host.
