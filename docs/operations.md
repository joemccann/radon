...
**Every cycle prunes `~/radon-weekend` before it exits.** The last thing each wrapper does, after the final phase has reported and regardless of that phase's status, is call `scripts/weekend_prune.py --root ~/radon-weekend` under a bounded `timeout` (`RADON_WEEKEND_PRUNE_TIMEOUT_SECS`, default 600s). The step is an **allowlist**: `CATEGORIES` names the only things it may delete (`__pycache__`, `.pytest_cache`, per-phase run logs older than 30 days, fully pushed and clean git worktrees, abandoned pytest tmp trees) and `refusal_reason()` is re-checked immediately before every unlink, so a category that ever enumerates a protected path still cannot delete one. Refused by construction: any `web/node_modules` (deleting one breaks the next remediate phase's vitest run), any `venv-*`, any `.deepsec` export or private `*scratch*` dir, a loop clone directory itself, anything outside the root and the OS temp dir, a worktree with unpushed or uncommitted work, and every path under a clone whose `.weekend-runner.lock` is held by a live pid. It reports bytes per category plus free space before and after into the phase log, its exit code is discarded, and `RADON_WEEKEND_SKIP_PRUNE=1` turns it off. Contract: `scripts/tests/test_weekend_prune.py`.
