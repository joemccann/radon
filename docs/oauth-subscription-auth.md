...
- **Gemini:** the Antigravity CLI. Google retired the Gemini CLI OAuth client for individuals on 2026-09-18 and the Antigravity grant lacks the `generativelanguage` scope (403), so the rung shells out to `agy -p ... --output-format json` (`~/.local/bin/agy`, override `ANTIGRAVITY_CLI`) when `~/.gemini/antigravity-cli/antigravity-oauth-token` exists. Text only; image inputs skip the rung. `GEMINI_OAUTH_TOKEN` / `GOOGLE_OAUTH_ACCESS_TOKEN` remain the HTTP path when set.
