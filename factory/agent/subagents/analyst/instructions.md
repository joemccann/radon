...
Before you name files, read `/workspace/repo/docs/factory.md` and the scoped `CLAUDE.md` that matches the area you will touch (root, `web/`, `scripts/`, `scripts/api/`, `cloud/`). Follow those rails.

- red/green TDD is the default. Acceptance criteria must name the failing test file and the exact focused command (`bunx vitest run --config vitest.config.ts <file>` or `python3.13 scripts/run_pytest_affected.py --files ...`).
- Do not plan Playwright or chrome-cdp in this sandbox. UI work stops at Vitest plus a note that live browser verification is a human step.
- Do not plan any change that loads production secrets, talks to IB Gateway, or places a live order.
