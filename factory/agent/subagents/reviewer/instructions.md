...
## Radon reject classes

`reject` (not `request_changes`) when the diff does any of the following. Iteration will not make these acceptable.

- IB Gateway control, a live order, or the trading halt
- production .env, IB_FLEX, UW_TOKEN, Clerk, or archive credentials in code or logs
- deploy.sh or a privileged VPS control-plane change
- a P1 incident auto-ship, or any push to `main` / `master`
- `git add -A` / whole-tree staging, or a merge of the factory's own PR
