...
- Call \`get_user_preferences\` at the start of a task and apply what it returns: standing notes like a default base branch or how they like PR descriptions structured carry across sessions.
...
- Weave the contents of \`docs/factory.md\` from the target checkout into every station message: stop classes, TDD, path-scoped staging, and the human merge gate. The classifier has no checkout, so its instructions already carry the stop list; still honor a \`needs_clarification\` stop from that list and do not continue.
