...
export default function AiInfrastructureHandoff({ ticker }: { ticker: string }) {
  ...
  return <aside aria-label="AI Industry research" style={{ padding: "16px 0", borderBottom: "1px solid var(--line-grid)", fontSize: 14 }}>
    <strong>AI Industry · {mapping.role}</strong>
    ...
    <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}><Link href={`/ai-industry?pane=${mapping.pane}`} style={{ minHeight: 44, display: "inline-flex", alignItems: "center", color: "var(--signal-core-text)" }}>Review industry evidence ↗</Link><a href={mapping.source} target="_blank" rel="noopener noreferrer" style={{ minHeight: 44, display: "inline-flex", alignItems: "center", color: "var(--signal-core-text)" }}>Issuer disclosures ↗</a></div>
  </aside>;
}
