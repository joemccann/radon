...
function SelectedMeasure({ indicator, sources, inspect }: { indicator: AiIndicator; sources: AiSource[]; inspect: (opener: HTMLButtonElement) => void }) {
  ...
  const coverage = indicator.history_coverage?.find(item => item.source_id === points?.[0].source_id && item.series_id === points?.[0].series_id && item.unit === points?.[0].unit);
  const copy = measureCopy(indicator);
  return <article className={styles.selectedMeasure} aria-label={`${copy.name} evidence`}>
    ...
    {points ? <AiIndustryHistoryChart key={`${points[0].source_id}-${points[0].series_id}-${points[0].unit}`} points={points} cadence={source?.cadence} sourceLabel={source?.name} observationCount={coverage?.observation_count} /> : <p className={styles.empty}>No comparable history is available. Missing observations are not zero.</p>}
    {indicator.history_series_count && indicator.history_series_count > groups.length ? <p className={styles.note}>{groups.length} of {indicator.history_series_count} stored series shown, prioritized by the latest reported measurements. Source totals cover the entire publisher.</p> : null}
...
export function AiInfrastructureView({ data, error, loading, refresh }: { data: AiSnapshot | null; error: string | null; loading: boolean; refresh: () => void }) {
  ...
  const measures = data?.indicators.filter(indicator => indicator.id !== "M1") ?? [];
  const available = measures.filter(indicator => indicator.status === "available").length;
  const sources = (data?.sources ?? []).filter(source => source.id !== "ib" && source.id !== "uw");
  ...
  return <section className={styles.workspace} data-testid="ai-infrastructure-panel" aria-busy={loading}>
    ...
    <div className={styles.snapshotMeta}><span>{measures.length} measures</span><span>{sources.length} sources</span><span>Snapshot {aiObservationDate(data?.as_of)}</span></div>
