...
test.describe("AI industry value chain", () => {
  ...
  test("coverage search, missing observations, stale evidence and failed refresh are explicit", async ({ page }) => {
    ...
    await expect(page.locator(".toast-container").getByRole("alert")).toContainText("temporarily unavailable");
    await expect(page.locator(".toast-container").getByRole("alert")).toContainText("Showing the last available data");
  });
