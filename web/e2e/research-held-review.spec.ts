...
const held = [
  { workKey: KEY, fileName: "jpm_flows___liquidity.pdf", publisher: "J.P. Morgan", series: "jpm flows liquidity", docType: "research", folderDate: "2026-09-17",
    ...
    context: { pageCount: 14, figureCount: 6, dateSource: "text", excerpt: "# Market Watch\n\nFlows & Liquidity. How much upward pressure on global bond yields from tech bond issuance? The $260bn increase in net bond issuance by tech companies for 2026 looks manageable.",
      selectorReason: "One measured finding on tech issuance and global yields. The labor market remains the swing factor through year-end.", sourceUrl: "/api/newsfeed/research/files/" + "c".repeat(64) + ".pdf" } },
...
for (const width of [1440, 393]) {
  test(`operator reviews held research at ${width}px and marks one should-have-published`, async ({ page }, testInfo) => {
    ...
    await expect(card.getByRole("heading", { name: "Market Watch" })).toHaveCount(0);
    await expect(card.getByText("Flows & Liquidity. How much upward pressure on global bond yields from tech bond issuance? The $260bn increase in net bond issuance by tech companies for 2026 looks manageable.")).toHaveCount(0);
    const note = card.getByTestId("held-selector-note");
    await expect(note).toBeVisible();
    await expect(note).toContainText("Selector's note:");
    await expect(note).toContainText("One measured finding on tech issuance and global yields. The labor market remains the swing factor through year-end.");
