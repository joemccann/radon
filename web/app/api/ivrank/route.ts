...
const CACHE_PATH = join(process.cwd(), "..", "data", "ivrank.json");

// Contract: absent ivrank data is HTTP 200 with missing:true, never a 4xx.
// The job's own degradation states ("degraded_uw", "stale_source") are real
...
async function readIvrankFromDb(): Promise<TimestampedRead<Record<string, unknown>> | null> {
  const db = getDb();
  const result = await db.execute({
    sql: `SELECT scan_time, payload FROM scan_snapshots
          WHERE service = 'ivrank' ORDER BY scan_time DESC LIMIT 1`,
    args: [],
  });
...
async function readIvrankFromDisk(): Promise<TimestampedRead<Record<string, unknown>> | null> {
...
export async function GET(): Promise<Response> {
  ...
  const result = await dbFirstRead({
    fromDb: readIvrankFromDb,
    fromDisk: readIvrankFromDisk,
    maxAgeMs: IVRANK_MAX_AGE_MS,
    label: "ivrank",
    // A fresher row that only carries the missing:true heartbeat must not
    // outrank an older row with a real series. The job's "degraded_uw" and
    // "stale_source" payloads are NOT degraded by this test: they carry a
    // full series and reach the client verbatim.
    isDegraded: isMissingPayload,
  });
  const response = NextResponse.json(result.ok && result.fresh ? result.data : MISSING_IVRANK);
