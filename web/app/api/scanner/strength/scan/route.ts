...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      res.headers.set("X-Sync-Warning", "Radon API unavailable - serving cached strength confirmation data");
      ...
      return setNoStoreResponseHeaders(
        NextResponse.json({ ...emptyStrengthConfirmationPayload(), error: message }, { status }),
        requestId,
      );
    }
  }
