...
describe("REGIME_RAIL_GROUPS — grouped registry covers every tab exactly once", () => {
  ...
  it("flattens to all 27 regime tabs with no duplicates", () => {
    expect(REGIME_TABS).toHaveLength(27);
    expect(new Set(REGIME_TABS).size).toBe(27);
    expect([...REGIME_TABS].sort()).toEqual(
      ["cri", "vcg", "gex", "grg", "breadth", "trin", "divyield", "hyad", "hhlev", "bpi", "margin", "credit", "iei-hyg", "straddle", "cor", "vixcor", "vixts", "dispersion", "ivrank", "skew", "skew2d", "curve", "cot", "ats", "short", "llm", "backtest"].sort(),
    );
  });
...
describe("RegimeRail — grouped rail rendering + navigation", () => {
  ...
  it("renders all five group headers and 27 items", () => {
    const { container } = renderRail();
    for (const g of REGIME_RAIL_GROUPS) {
      expect(within(container).getByText(g.label)).toBeTruthy();
    }
    expect(container.querySelectorAll("[data-tab]")).toHaveLength(27);
  });
