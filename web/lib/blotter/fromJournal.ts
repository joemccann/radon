...
export function compactExpiry(expiry: JournalTradePayload["expiry"]): string {
...
// Also consumed by lib/journal/realizedPnl.ts (assistant journal tools).
export function contractGroupKey(p: JournalTradePayload): string | null {
  const ticker = resolveTicker(p);
