...
export function userErrorMessage(error: unknown, fallback = DEFAULT_MESSAGE): string {
  ...
  const status = Number(/(?:\bHTTP(?: error)?|\bRadon API|\bstatus(?: code)?)[ :]*(\d{3})\b/i.exec(message)?.[1]);
  ...
  if (status === 429 || /rate.?limit|too many requests|quota (?:exceeded|exhausted)/i.test(message)) return "Too many requests. Please wait before trying again.";
  if (status === 408 || status === 504 || /timed?\s*out|timeout/i.test(message)) return "The request took too long. Please try again.";
  if (!Number.isFinite(status) && /failed to fetch|fetch failed|network (?:error|request failed)|ECONN\w*|offline/i.test(message)) return "Unable to connect. Check your connection and try again.";
  if (status === 401 || /unauthorized|session expired/i.test(message)) return "Your session has expired. Sign in again to continue.";
  if (status === 403 || /forbidden/i.test(message)) return "You do not have access to this request.";
  if ([500, 502, 503].includes(status) || /bad gateway|internal server error|service unavailable/i.test(message)) return "This service is temporarily unavailable. Please try again shortly.";
