...
// R-666: production-absence assertion for the Playwright authless bypass.
// The bypass is token-bound, but the FLAG must still never reach a real
// deployment: a live-Clerk instance (pk_live_) with RADON_AUTHLESS_TEST=1
// fails the boot rather than serving. E2E runners are keyless or pk_test,
// so they never trip this.
export function assertAuthlessTestFlagAbsentInProduction(
  ...
  if (flag === "1" && publishableKey?.startsWith("pk_live_")) {
    throw new Error(
      "RADON_AUTHLESS_TEST=1 is set on a live-Clerk production deployment; " +
        "the authless test bypass must never ship — unset the flag and redeploy",
    );
  }
}
assertAuthlessTestFlagAbsentInProduction();
