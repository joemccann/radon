...
| **Dashboard** (overhauled 2026-08-09 from the Claude Design mock) | `dashboard/DashboardSurface.tsx`, `KpiStrip.tsx`, `FeedPanel.tsx`, `ScannerHero.tsx`, `CatalystsQuadrant.tsx`, `EngineStatePanel.tsx`, `DashboardNewsFeed.tsx` (mobile feed), libs `dashboardKpis.ts`, `scannerHero.ts`, `catalystGroups.ts`, `engineState.ts`, `useNewsfeedPosts.ts` | **KPI strip + 1.2fr/1fr grid.** Strip: Net Liq / Today P&L / Buying Power (bar = available_funds/EWL) / Margin Used (maint/NLV, tone via `assessMargin`). Left: `FeedPanel` featured story + headline list (click swaps featured; mobile keeps `DashboardNewsFeed`). Right: `ScannerHero` theta/strength pill tabs (only active tab polls; other scans on `/scanner`) above the quadrants: `CatalystsQuadrant` (TODAY / THIS WEEK / POSITIONS join vs open positions) and `EngineStatePanel` (CRI · MARKOV · VCG · GEX, real payload-derived readings; missing data = `P ---`, never fault). Section ids `feed/signals/catalysts/engine`; DashboardSurface takes `marketState`, orders prop removed. |
