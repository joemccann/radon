...
| **Options Chain sticky header** | `OptionsChainTab.tsx` | Three required CSS rules: `background: var(--bg-panel-raised)` on `.chain-header`+`.chain-side-label`; `position: sticky; top: 0`/`top: 24px`; `.chain-grid thead { position: relative; z-index: 10 }`. `prefillLabelForSource` maps `src`: `vol-cone` -> `PREFILLED FROM VOL CONE`, `leap` -> `PREFILLED FROM LEAP SCAN`, anything else -> `PREFILLED FROM THETA HARVESTER`. The label only renders when the URL also carries `legs`, so the ticker-level scanner links (`src=flow|discover|strength|garch`) never reach it - those payloads name no contract, so they open the chain ladder with an empty ticket. |
