...
- **Depth subject ownership.** `TickerDetailContent`'s effect is the ONLY writer of `depthSymbols` (publishes `[bookKey]`, cleanup publishes `[]`). `setActiveTicker` must never reset it: React runs child passive effects before the parent's, so a reset inside the shell's ticker-sync lands AFTER the new subject's publish, `usePrices` never sends `subscribe-depth`, and client-side ticker navigation degrades to a single-row "L1 BBO" book with an empty tape while a hard load looks fine (2026-08-24). Regression test: `web/tests/ticker-detail-context-depth-symbols-race.test.tsx`; e2e: `web/e2e/book-depth-client-nav.spec.ts`.
