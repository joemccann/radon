...
Every `web/app/api/**/route.ts(x)` must `export const radonCapability` (`read` | `read.spawn` | `mutate.workspace` | `mutate.trading` | `admin` | `internal`, or a method map of those). Unclassified routes fail `web/tests/assistant-catalog-pin.test.ts`. New GET indicator routes are usually `"read"`. Chat `list_apis` / `call_api` are derived from those pins plus `scripts/api/assistant_catalog.py` (freshness: `web/tests/assistant-catalog-freshness.test.ts`). Do not add a third handwritten catalog. FastAPI twins (example: `/api/streaks` vs `/streaks/{ticker}`) are advertised as the FastAPI path. Next-only routes need a static loader in `web/lib/assistant/nextLoaders.ts`. A callable pin (`read` / `read.spawn` / `mutate.workspace`) auto-exposes the route to chat `call_api`: the only exclusion is `DENY_PREFIXES` in `web/lib/assistant/catalogBuild.ts`, and FastAPI `mutate.workspace` pins are operator-gated automatically (`isOperatorOnly`). Pin `internal`, `admin`, or `mutate.trading` when chat must never call the route.
...
11. **Quote-staleness submit gate (REL-236).** `lib/order/quoteSubmitGate.ts` disarms transmit when the quote timestamp is older than `QUOTE_SUBMIT_MAX_AGE_MS` (5m, matching the telemetry CLOSE relabel) or `feedConnected === false`; a missing or unparseable timestamp leaves the gate open. Wired into `SingleLegOrderTicket` and `ModifyOrderModal`, on top of `okToSubmit` and the unbounded-loss acknowledgement — a surface passing those two can still be disarmed here.
...
**Mixed-age combos are not same-day** (2026-09-17). Overnight long + same-session short (SPY 740P held since May, 760P sold today → bull put spread) stamps `basis_source: mixed` and often `entry_date=today`. The same-day identity then prints the overnight leg's entire accumulated P&L as Today P&L (−$162k). `isSameDay` is false for mixed; Today P&L is per-leg: overnight vs prior close, `session_fills` vs fill. Prefer `ib_daily_pnl` when IB already mixed-lot summed. Without that total, require a usable mark and baseline for every leg; any missing contribution makes Today P&L unavailable, never a partial sum. Tests: `mixed-age-combo-today-pnl.test.ts`.
...
**A non-positive `last` is not a price.** `resolveSingleLegLastPrice` (`web/lib/orders/orderDisplay.ts`) rejects zero/negative lasts instead of rendering them; a mark that fell back to the previous session's close carries `isPreviousClose: true` and must render visibly marked as such, never as a live last (06b77c64, R-607/R-608).
...
Stock→equity. Long Call/Put→defined. Short Call/Put→undefined. Spreads→defined. Synthetic/Risk Reversal→undefined. Long Straddle→defined. Covered Call→defined. All-long combo→defined. Mixed-direction all-option combo where, per right, long qty ≥ short qty→defined, with exact expiry max loss from `_defined_option_combo_max_risk` (piecewise-linear evaluation at spot 0 and every strike; `scripts/ib_sync.py`, ca853a3c). Unrecognized→complex (→Undefined Risk table).
...
| **Regime day-change** | `.regime-strip-day-chg` | VIX/VVIX/SPY: WS `last` vs the previous SESSION close, resolved by `resolvePreviousSessionClose()` off `data.history` and anchored to `lastCompletedSessionDate()`; the WS `close` is the fallback only when the payload has no history. Never `data.{vix,vvix,spy}` — that is the scan-time SPOT, and using it rendered VIX -11.35% on a +1.65% day (2026-08-28). A history too old to reach the session yields no baseline (R-200). RVOL: `intradayRvol - data.realized_vol`. COR1M: `data.cor1m_5d_change`. Arrow right of change via `display: flex; gap: 4px`. |
...
| **Options Chain sticky header** | `OptionsChainTab.tsx` | Three required CSS rules: `background: var(--bg-panel-raised)` on `.chain-header`+`.chain-side-label`; `position: sticky; top: 0`/`top: 24px`; `.chain-grid thead { position: relative; z-index: 10 }`. `prefillLabelForSource` maps `src`: `vol-cone` -> `PREFILLED FROM VOL CONE`, `leap` -> `PREFILLED FROM LEAP SCAN`, `theta`/`theta-harvester` -> `PREFILLED FROM THETA HARVESTER`, anything else -> `PREFILLED FROM LINK`. The neutral fallback is deliberate (R-414): the old default attributed an ABSENT or misspelled `src` to Theta Harvester, so a hand-edited or shared `?legs=` URL carried a scanner verdict no scanner produced - `thetaOrderHref` therefore now emits `src=theta` to keep its own label. The label only renders when the URL also carries `legs`, so the ticker-level scanner links (`src=flow|discover|strength|garch`) never reach it - those payloads name no contract, so they open the chain ladder with an empty ticket. |
...
| **Fill Toasts** | `lib/fillToasts.ts` (pure), `lib/useFillToasts.ts`, mounted in `WorkspaceShell` | Toasts are COALESCED per order via `upsertToast` (one running toast per order group, message updated in place — not one toast per execution); the `onNewFills` callback wired in `WorkspaceShell` to `portfolioSyncNow` is a load-bearing seam — removing it leaves the portfolio stale after a fill (88dd94ab, aad3e06b; tests `use-toast-upsert.test.tsx`, `fill-driven-portfolio-refresh.test.tsx`). Dedupe key strips IB execId correction suffix; first payload of a browser session primes silently (no load storm); seen set persists to sessionStorage; route-nav remounts restore the baseline and DIFF their first payload so mid-navigation fills still toast. Demo suppressed. sessionStorage access is SecurityError-safe. Tests: `fill-toasts*.test.*`. |
...
| **Cash Flows** (on `/orders`) | `scripts/cash_flow_sync.py`, `useCashFlows.ts` | IBKR `CashTransaction` rows (deposits/withdrawals/dividends/interest/fees/withholding). Reads `IB_FLEX_NAV_QUERY_ID`. Idempotent on `transactionID`. **Cadence: the sFTP-delivered Activity statement, `radon-flex-pull.timer` Tue..Sat 07:30 ET -> `flex_delivery_ingest` -> `cash_flow_sync --from-file`, the only path that writes `cash_flows`. The monitor daemon's `CashFlowSyncHandler` is NOT registered since 2026-09-02 (a weekday SendRequest is off by policy).** Throttle backoff on Flex 1018 only (90s→5m→15m→1h capped); 1001/1009 take the bounded soft lane, 1019 is not-ready. **Sync lozenge**: route surfaces `last_synced_at` + `sync_status` from `service_health.cash-flow-sync`, written by that ingest (`ok` after a successful `--from-file` run or an already-applied duplicate statement, `error` with the exit code when it fails); amber when `is_throttled`, red `--fault` for other errors, calm `--ok` when healthy. Don't manually retry — every Flex request during throttle pushes the reset further out. **1025** ("Too many failed attempts") is an undocumented token lockout, not 1018: exit 15, 7-day shared sidecar (`utils.flex_embargo`), TWR and `/performance/background` must not SendRequest. Recover with `--from-file`. |
...
| **Mobile shell** (PWA, 393×852) | `useViewport.ts`, `components/mobile/`, `PwaRegister.tsx`, `public/{manifest.webmanifest,sw.js}` | `useViewport()` (≤640 / 641-1023 / ≥1024) drives `<MobileShell>` from `WorkspaceShell` when `isMobile && hasMounted`. Sets `body[data-mobile="true"]`. Manifest standalone, Clear dark theme #101714. Hand-written SW is network-first with offline fallback (`sw-decisions.js` allowlist); `/ws`, `/health`, `/_next/data`, non-GET and non-allowlisted `/api` stay untouched. |
...
- **Selected web design (2026-09-05): A / Clear.** The user explicitly replaced the prior web visual constraints. `DESIGN_MEMORY.md`, `app/globals.css` and `app/clear.css` define the white/evergreen system, quiet dark counterpart, horizontal desktop navigation, four-destination mobile navigation, Inter hierarchy, 12px metadata floor and restrained 6/8/10px corners. Existing trading, authentication and realtime invariants are unchanged. The dashboard is account-first with a risk/research rail; all previous market-intelligence sections and visibility controls remain below. This rule supersedes older visual/dashboard descriptions above, not functional contracts.
...
**Ownership (2026-09-01): the ONE `usePrices` instance lives in `RealtimePricesProvider` (`lib/RealtimePricesContext.tsx`, mounted in root `Providers`) — never in the per-page `WorkspaceShell`.** App Router remounts pages on every navigation, so a shell-owned socket died on each route change: close → fresh ws-ticket → reconnect → full resubscribe → snapshot resync (the mobile "every page change lags" bug). Shells publish their desired set via `publishSubscriptions`: content-diffed (publishers hand fresh array identities every render; identity-diffing loops provider setState → consumer re-render), last-write-wins, deliberately NO effect cleanup (a page swap must never empty the set mid-swap). Growth applies immediately; shrinks commit after a 5s linger (`SUBSCRIPTION_SHRINK_LINGER_MS`) so a remounting shell whose portfolio/orders are still re-resolving can't unsubscribe held symbols and resync them a second later; depth subjects pass through exactly (scarce ~3-ticket relay resource). `TickerSearch` connects on first focus, not on mount, for the same reason (the Header remounts per page and stays mounted-but-hidden on mobile). Ownership pin: `tests/realtime-socket-ownership-contract.test.ts`; behavior: `tests/realtime-prices-navigation-persistence.test.tsx`, `tests/ticker-search-lazy-connect.test.tsx`.
