...
**Auto-deploy on push to main.** `.github/workflows/ci.yml` runs the Vitest + pytest gate then deploys on green: it SSHes to Hetzner and runs `bash scripts/deploy.sh` from `~/radon-cloud/`. The deploy job carries the `Production` GitHub Environment as a non-blocking label (its 2026-07-03 manual-approval rule was removed at the operator's request on 2026-07-04; re-adding a required-reviewers rule in Settings > Environments re-gates deploys with no yaml change). A newer push cancels a run still in flight (concurrency group), so rapid pushes deploy only the last one. A newer push cancels a run still waiting at the gate (concurrency group), so rapid pushes deploy only the last one. Confirm: `gh run list --workflow=ci.yml --limit 1`. After deploy, `sudo systemctl restart radon-api.service` may be needed. `deploy.sh` lives in the separate `radon-cloud` repo (CI does NOT git-pull it; the VPS working copy is authoritative — edit there + push). It health-gates the relay restart: before tearing services down (while the current radon-api still serves `/health`, so the reading is the gateway's real state, not a just-restarted api's pool warmup), `wait_for_gateway_ready` confirms the IB gateway is authenticated + port_listening (bounded 60s, warn-and-proceed), so a deploy never restarts the relay into a mid-restart/awaiting-2FA gateway. The relay itself self-heals on reconnect and raises a `service_health` row (`ib-realtime-relay`) instead of looping silently on no-ticks (see `project_relay_stale_tick_ladder_2026_06_05`).
