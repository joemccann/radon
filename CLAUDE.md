...
**`IB_FLEX_NAV_QUERY_ID` was documented here as `1497709` until 2026-08-16 and that was wrong** — both the Hetzner env and the root `.env` carry `1442520`. Three scripts read that var (`cash_flow_sync.py`, `perf_twr_builder.py`, `portfolio_performance.py`), so the stale value silently pointed a runbook at a query that does not exist for this account. Verify against the real env before trusting a query id written down anywhere. `IB_FLEX_FLOWS_QUERY_ID` is deliberately unset: `_flows_query_id()` falls back to the NAV id and `resolve_flows` reuses the one fetched document, so a run makes ONE Flex request. Setting it to a second id doubles the request rate against a token that has already taken a 24h-to-168h throttle embargo.
