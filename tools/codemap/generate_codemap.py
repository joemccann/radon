...
Walks every source area (web/, site/, scripts/, cloud/, lib/, tests/), resolves
TS/JS and Python imports to repo-internal files, and emits:
...
SOURCE_ROOTS = ("web", "site", "scripts", "cloud", "lib", "tests")
...
def _js_candidates(base: Path) -> list[Path]:
    ...
    if base.suffix in (".js", ".mjs", ".cjs"):
        # ESM-style specifier written with a .js suffix pointing at a TS source.
...
def extract_py_deps(text: str, rel: str, index: set, externals: Counter) -> set:
    ...
    for module, names in PY_FROM_RE.findall(text):
        ...
        bare = module.lstrip(".")
        resolved_pkg = (
            resolve_py_relative(dots, bare, rel, index) if dots else resolve_py_module(bare, rel, index)
        )
        if resolved_pkg and resolved_pkg != rel:
            deps.add(resolved_pkg)
        if resolved_pkg is None and dots == 0:
            externals[bare.split(".")[0]] += 1
        # `from pkg import submodule` -- resolve each name as a child module too.
        for raw in names.replace("(", "").replace(")", "").split(","):
            ...
            target = (
                resolve_py_relative(dots, child, rel, index) if dots else resolve_py_module(child, rel, index)
            )
            if target and target != rel:
                deps.add(target)
    return deps
