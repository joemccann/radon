...
### 2026-09-16 - audit - branch `ci-performance/2026-09-16`

- Runner state: dedicated `.radon-weekend-runner` and `.radon-ci-performance-runner` markers, clean `origin/main` base, GitHub authentication, Python 3.13, Production custom-branch policy, and all 24 required main-protection contexts were verified. Stale `.weekend-runner.lock` PID `8134` was not live and was recoverably preserved at `/tmp/ci-performance-stale-lock-8134-1789542401` before exclusive lock acquisition.
- Audited range: `fe96fdac..8d1d0f52` (44 commits, 206 files). It changes web, cloud/control-plane, model-ladder, workflow-removal, generated codemap, docs, and nightly ledgers. The sole CI DAG delta, `.github/workflows/ci.yml:717-850`, adds a curated chain-anchor E2E and its screenshot upload inside the explicitly non-gating Playwright smoke job; it does not alter protected `deploy.needs`, cache keys, image build contexts, transfer/provenance, path-filter fallback, health, recovery, rollback, or stability behavior.
- Measurement: fetched 30 recent Actions runs, including 13 completed primary CI workflows. Nine ordinary/warm successful mixed/full deployments ([35048622543](https://github.com/joemccann/radon/actions/runs/35048622543), [35038433486](https://github.com/joemccann/radon/actions/runs/35038433486), [35029245850](https://github.com/joemccann/radon/actions/runs/35029245850), [35019111664](https://github.com/joemccann/radon/actions/runs/35019111664), [35012937987](https://github.com/joemccann/radon/actions/runs/35012937987), [35003654790](https://github.com/joemccann/radon/actions/runs/35003654790), [34978594818](https://github.com/joemccann/radon/actions/runs/34978594818), [34910390815](https://github.com/joemccann/radon/actions/runs/34910390815), [34909678726](https://github.com/joemccann/radon/actions/runs/34909678726)) have createdAt-to-successful-Deploy clocks of 280--365s, p50 325s/p95 365s. The docs/config run [35051076130](https://github.com/joemccann/radon/actions/runs/35051076130) is separately classified control-plane-only at 168s; cache/infrastructure-degraded [34911054382](https://github.com/joemccann/radon/actions/runs/34911054382) is 329s to successful Deploy and remains excluded from the ordinary comparison. No five-run cold-cache window exists; free-plan timing reports no billed minutes, so outcome remains `INSUFFICIENT_SAMPLE`.
- Current representative DAG: [35048622543](https://github.com/joemccann/radon/actions/runs/35048622543) completed the exact SHA `95fd0430f30fbf4996e7716c0796cd99b82cb536` in 281s: Path filter 8s, `scripts-gh` 119s / `scripts-rs` 113s plus node image 104s, coverage fan-in 14s, parallel prestage/prepull 26s/19s, then Deploy 102s. First-job queue was 3s. In [35038433486](https://github.com/joemccann/radon/actions/runs/35038433486), node image 95s and `scripts-rs` 114s co-walled before 91s Deploy; in [35029245850](https://github.com/joemccann/radon/actions/runs/35029245850), node image 107s and `scripts-rs` 112s co-walled before 96s Deploy. Queue remains separately reported, not a source-performance gain.
- Safety/DAG sweep: `.github/workflows/ci.yml:872-1058` keeps the identical full required-gate closure for prestage, prepull, and Deploy; Deploy remains non-canceling after the teardown boundary. `.github/workflows/app-images.yml:22-109` retains SHA-pinned actions, isolated GHA cache scopes, and exact-SHA pair tags. Recursive shard-union and coverage contracts, fail-closed path classification, and immutable artifact handling remain intact. `cloud/scripts/deploy.sh:83-85,1448-1505,1603-1692` retains the 40-second NRestarts window, health gates, transition recovery, rollback, and green marker.
- Candidate ranking: no `CIP-014` allocated. The added non-gating E2E/upload is off the release path. The measured `scripts-rs`/node-image co-wall remains work-bound; cache-warm image publication and ordinary variation do not prove a source-controlled recurring >=15s/10% gain. Repartitioning risks recursive shard-union completeness and runner-minute growth; cache, transfer, duplicate-setup, path-filter, cancellation, provenance, prestage/prepull, recovery, and rollback sweeps found no new actionable defect. Expected saving <15s, confidence low, risk/validation cost high, runner-minute impact zero. Outcome: `NO_SAFE_CHANGE` / `INSUFFICIENT_SAMPLE`; CIP-005/CIP-007/CIP-009/CIP-013 remain `VALIDATING`, CIP-004/CIP-006/CIP-008 remain `DEFERRED`.
- Verification: `../venv-ci-performance/bin/python -m pytest scripts/tests/test_ci_gate_integrity.py scripts/tests/test_ci_deploy_concurrency.py scripts/tests/test_path_filter.py scripts/tests/test_codemap.py -q` -- **119 passed in 17.99s**; Ruby YAML parsing of both workflows, `bash -n scripts/ci_performance_nightly.sh`, and both diff checks passed. No production action was invoked. Revert trigger: reject any future candidate that shrinks gate closure, inventory, coverage, path fallback, immutable provenance, health/recovery/rollback, or stability behavior.
- Publication: audit commit `98a960ad`, PR [#461](https://github.com/joemccann/radon/pull/461), and rolling issue [comment](https://github.com/joemccann/radon/issues/196#issuecomment-5693483095) are published; PR validation belongs to the later deliver phase.

### 2026-09-16 - remediate - branch `ci-performance/2026-09-16`

- Runner state: dedicated markers, the dated audit branch, GitHub authentication, `origin/main`, the audit's Production-protection closure, and exclusive runner-lock ownership were re-verified. `RADON_WEEKEND_REDUCED=1` confines this phase to verified P0/P1 findings.
- Remediation eligibility: the completed audit supplies zero P0/P1 source-actionable findings and no `CIP-014` allocation. The measured `scripts-rs`/node-image co-wall has no demonstrated safe recurring >=15-second/10% reduction; cache-degraded variance has no comparable source-controlled sample. No lower-priority candidate was substituted.
- Verification: `../venv-ci-performance/bin/python -m pytest scripts/tests/test_ci_gate_integrity.py scripts/tests/test_ci_deploy_concurrency.py scripts/tests/test_path_filter.py scripts/tests/test_codemap.py -q` passed **119 tests in 15.57s**; both workflow YAML files parsed, `bash -n scripts/ci_performance_nightly.sh`, and base/branch diff checks passed. A detached serial broader baseline prewrote `scripts/cloud/root/vitest` result slots but produced no `DONE` sentinel; it is explicitly not counted as a passing gate.
- Safety/impact: no test inventory, coverage, path classification, gate dependency, immutable pin, cache/artifact provenance, exact-SHA verification, health, recovery, rollback, cancellation, or 40-second stability behavior changed. Runner-minute impact is zero. Outcome: `NO_SAFE_CHANGE` / `INSUFFICIENT_SAMPLE`; CIP-005/CIP-007/CIP-009/CIP-013 remain `VALIDATING`, CIP-004/CIP-006/CIP-008 remain `DEFERRED`. Residual bottleneck remains the required gate/image co-wall then protected Deploy floor. Revert trigger: reject any candidate that weakens a protected rail.
