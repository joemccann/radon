...
3. **Never push to `main`.** Actual changes use
   ...
   `Documentation <YYYY-MM-DD>`. Human merge remains the only delivery path.
...
10. Open or update the PR with the trigger, stale claim, source-of-truth
    evidence, value classification, owner, deletion/consolidation performed,
    validations, and any exact operator action.
...
- required focused checks are green, and code-bearing changes pass the full
  project suites;
- the PR states every P0/P1, source evidence, owner, recurrence guard,
  validation counts, and `OPERATOR_REQUIRED` item.
