...
2. **Take an exclusive loop lock.** Use namespaced scratch and state outside
   the repository. Never reset, clean, modify, or kill work owned by another
   process. Serialize CPU-, memory-, and model-heavy work with the shared
   Mac mini heavy-work semaphore.
...
1. `# security-deepsec <phase> <run-id>` with `HEAD_SHA`, `LAST_AUDITED_SHA`, the
