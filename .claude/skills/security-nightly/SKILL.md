...
wrong, as one bullet per finding: `- **Component**: what happened.`,
sanitized), `--fix` (what this PR actually changed, one bullet per fix,
same shape, sanitized), and `--next` only when something still must happen
outside of CI pushing a new deployment (bulleted the same way when there's
more than one). Omit `--next` and the formatter emits
`Fixed with green deployment`. A single plain sentence still works when
