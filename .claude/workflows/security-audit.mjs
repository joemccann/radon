...
⚠️ THE GITHUB REPO IS PUBLIC (radon). Anything committed — including deleted content still in git history — is world-readable. A prior sweep (2026-07-05) found cleartext creds in history that must be rotated. Treat any secret-shaped string reachable anywhere in git history as EXPOSED, not merely a hygiene nit.
...
RUN THESE FRESH (don't trust cached counts): cd ${REPO}/web && npm audit --omit=dev --json ; committed-secret checks go through redacted gitleaks metadata only (see the secrets dimension; never read raw history or a secret value) ; rg for the sinks named in your dimension.
...
const DIMENSIONS = [
  ...
  { key: 'secrets', label: 'secrets', scope: `Secrets & credential exposure. THE REPO IS PUBLIC — a committed secret is a live disclosure. Read .env.example, web/.env.example, scripts/api/tests/test_no_secret_leakage.py. rg for: console.log/logger of tokens/passwords/auth headers, non-NEXT_PUBLIC secrets referenced in 'use client' code or returned in API JSON, error responses echoing env/stack/paths, secrets in URLs, /api/flex-token exposure, Clerk secret vs publishable, and the NEW RADON_ARCHIVE_S3_* B2 creds (must never be echoed by archive_portfolio_snapshots.py logs or committed). Check committed secrets, current tree AND history, ONLY through redacted gitleaks metadata; never read raw history (no 'git log' with patches), never grep for secret values, never print a Secret or Match field: run 'gitleaks git ${REPO} --config ${REPO}/cloud/.gitleaks.toml --redact --no-banner --report-format json --report-path /tmp/radon-gitleaks.json' then read ONLY 'jq "[.[] | {RuleID, File, StartLine, Commit}]" /tmp/radon-gitleaks.json' (drop the Secret and Match fields even though --redact masks them) and delete the report. Cite findings as RuleID + File:StartLine + Commit. The 2026-07-05 sweep confirmed history still contains creds pending rotation: report whether those RuleID/File/Commit hits still resolve, never their values.` },
