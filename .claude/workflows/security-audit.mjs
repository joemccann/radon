...
THE GITHUB REPO IS PUBLIC (radon). Anything committed, including deleted content still in git history, is world-readable. History exposure is decided by the pinned gitleaks run with --redact. Do not read commit patches. Never quote a secret value.
...
RUN THESE FRESH (don't trust cached counts): cd ${REPO}/web && npm audit --omit=dev --json ; rg for the sinks named in your dimension. Do not read commit history.
  ...
  properties: {
    findings: { type: 'array', items: {
      ...
      properties: {
        ...
        description: { type: 'string', description: 'NEVER reproduce a credential literal here - name the variable and cite file:line instead. Reports get committed; quoting the value makes the report the leak.' },
        exploitability: { type: 'string', description: 'concrete attack path, or why it is only defense-in-depth' },
        evidence: { type: 'string', description: 'cite file:line and the surrounding shape. NEVER paste a secret value - the renderer redacts it, but it should not be in the JSON either.' },
...
const DIMENSIONS = [
  ...
  { key: 'secrets', label: 'secrets', scope: `Secrets and credential exposure. THE REPO IS PUBLIC. A committed secret is a live disclosure. History is owned by the pinned gitleaks contract (gitleaks detect --redact). If args.gitleaksRedacted is present, adjudicate only those redacted rows: rule id, file, line, commit, fingerprint. The secret and match fields are already removed. If it is absent, produce those rows yourself and read nothing else: run 'gitleaks git ${REPO} --config ${REPO}/cloud/.gitleaks.toml --redact --no-banner --report-format json --report-path /tmp/radon-gitleaks.json', read ONLY 'jq "[.[] | {RuleID, File, StartLine, Commit, Fingerprint}]" /tmp/radon-gitleaks.json' (never the Secret or Match fields, even though --redact masks them), then delete the report. Do not read commit patches or history, and do not ask for the secret text. Current tree only: read .env.example and web/.env.example for key NAMES, and scripts/api/tests/test_no_secret_leakage.py. rg source for loggers, error responses, URL builders, and 'use client' modules that interpolate secret variable NAMES (tokens, passwords, auth headers, Clerk secret, RADON_ARCHIVE_S3_*). Never open .env, web/.env, or a git patch. A current-tree finding cites file:line and the variable name.` },
