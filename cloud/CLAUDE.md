...
is recorded, repeated recovery does not restart a completed dump. Restore
verification accepts an in-progress backup or `inactive` with `Result=success`,
and rejects failures. The real backup completion still owns its health row;
deploy does not manufacture a healthy heartbeat.
